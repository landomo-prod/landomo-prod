...
/**
 * DetailView - Shared sliding detail panel for all desktop views
 *
 * Features:
...
export function DetailView({
  ...
  const { property: detailResult } = useProperty(
  ...
  return (
    <div
      className={`bg-white flex flex-col h-full transition-all duration-300 ease-out overflow-hidden ${
        inline
          ? "flex-1"
          : overlay
          ? "absolute right-0 top-0 bottom-0 z-50 border-l border-gray-100 shadow-2xl " + (isOpen ? "w-[520px] lg:w-[640px] xl:w-[780px]" : "w-0 border-l-0")
          : "flex-shrink-0 border-l border-gray-100 shadow-2xl " + (isOpen ? "w-[520px] lg:w-[640px] xl:w-[780px]" : "w-0 border-l-0")
      }`}
    >
      <div ref={scrollRef} className="flex-1 overflow-y-auto custom-scroll relative">
        ...
        <div className="sticky top-0 z-20 h-0 overflow-visible pointer-events-none">
          <div className="flex items-center justify-between px-4 pt-3 pointer-events-none">
            <button
              onClick={onClose}
              className={`pointer-events-auto h-10 w-10 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 ${
                scrolledPastImage
            ...
            <div className="flex gap-2 pointer-events-auto">
              <button
                ...
                className={`h-10 w-10 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 ${
                  scrolledPastImage
      ...
      {/* Image Gallery — carousel main + rotating thumbnails */}
      <div className="relative p-4 flex-shrink-0">
        <div className="flex gap-3 h-[280px] xl:h-[320px]">
          {/* Main image — card-style carousel */}
          <div className="flex-[3] relative bg-gray-100 rounded-2xl overflow-hidden">
            <Image
            ...
            <div className="absolute inset-0 flex">
              {images.length > 1 && (
                <div className="w-1/3 cursor-pointer" onClick={handlePreviousImage} />
              )}
              <div className="flex-1 cursor-pointer" onClick={() => setLightboxOpen(true)} />
              {images.length > 1 && (
          ...
          {/* Thumbnails — always show the other images, rotating with currentImageIndex */}
          {images.length > 1 && (() => {
            const others = images
              .map((img, idx) => ({ img, idx }))
              .filter((item) => item.idx !== currentImageIndex);
            const visible = others.slice(0, 5);
            const overflowItem = others[5];
            const overflowCount = others.length - 5;
            return (
              <div className="flex-[2] grid grid-cols-2 grid-rows-3 gap-3 h-full">
                {visible.map((item) => (
                  <div
                    key={item.idx}
                    className="relative bg-gray-100 rounded-xl overflow-hidden cursor-pointer"
                ...
                {overflowItem && (
                  <div
                    className="relative bg-gray-100 rounded-xl overflow-hidden cursor-pointer"
                    onClick={() => { setCurrentImageIndex(overflowItem.idx); setLightboxOpen(true); }}
                  >
                    <Image src={overflowItem.img} alt="More" fill className="object-cover" sizes="20vw" />
      ...
      <div className="px-5 pb-32">
          ...
          <div className="flex-1 flex flex-col gap-4">
            ...
            <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
              ...
              {property.source_url && (
                <button
                  ...
                  className="w-full bg-white text-gray-900 font-bold py-2.5 rounded-full border border-gray-200 hover:bg-gray-50 transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-xs"
                >
            ...
            {property.transaction_type === 'sale' && !isPriceOnRequest(property.price) && (
              <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                ...
                <button className="w-full bg-white text-[#84CC16] font-bold py-2.5 px-4 rounded-full border border-gray-200 hover:bg-lime-50 hover:border-purple-200 transition-all active:scale-[0.98] text-sm">
                  Find out more
                </button>
              </div>
            )}
        ...
        {/* Price History Chart - fetches real data */}
            ...
            <div className="relative">
              ...
              {carouselScrollPosition > 0 && (
                <button
                  ...
                  className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 backdrop-blur-sm p-2 rounded-full shadow-lg border border-gray-100 hover:bg-white transition-all active:scale-95"
                >
              ...
              {carouselScrollPosition < (similarProperties.length - 3) * 296 && (
                <button
                  ...
                  className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 backdrop-blur-sm p-2 rounded-full shadow-lg border border-gray-100 hover:bg-white transition-all active:scale-95"
                >
      ...
      <div className="flex-shrink-0 flex items-center justify-center gap-3 px-4 py-4 border-t border-gray-100 bg-white">
        <button className="rounded-full bg-[#84CC16] px-8 py-3.5 text-sm font-black text-white shadow-lg shadow-lime-100 hover:bg-[#6aaa10] active:scale-95 transition-all">
          Book viewing
        </button>
        <button className="h-12 w-12 rounded-full border border-gray-100 bg-gray-50 flex items-center justify-center hover:bg-gray-100 active:scale-95 transition-all">
