 ...
 */
export const SYSTEM_PROMPT = `Extract structured property data from Czech real estate listings.
 ...
 */
export function validateExtraction(data: LLMExtractedProperty): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  // Required fields
  if (!data.property_type) errors.push('property_type is required');
  if (!data.transaction_type) errors.push('transaction_type is required');
  if (!data.extraction_metadata?.confidence) errors.push('extraction_metadata.confidence is required');
