...
import { detectPropertyCategory, getCategoryConfidence } from '../utils/categoryDetection';
import { transformBazosApartment } from './bazosApartmentTransformer';
import { transformBazosHouse } from './bazosHouseTransformer';
import { transformBazosLand } from './bazosLandTransformer';
 ...
 */
export async function transformBazosListingByCategory(
  listing: BazosAd,
  country: string = 'cz'
): Promise<StandardProperty & Record<string, any>> {
  console.log(`[BazosTransformer] Processing ${listing.id}`);

  // Detect category
  const description = (listing as any).description || '';
  const category = detectPropertyCategory(listing.title, description);
  const confidence = getCategoryConfidence(listing.title, description);

  console.log(`[BazosTransformer] Category: ${category} (confidence: ${confidence.toFixed(2)})`);

  // Route to category-specific transformer
  let transformed: Partial<StandardProperty>;

  switch (category) {
    case 'apartment':
      transformed = await transformBazosApartment(listing, country);
      break;
    case 'house':
      transformed = await transformBazosHouse(listing, country);
      break;
    case 'land':
      transformed = await transformBazosLand(listing, country);
      break;
  }
  ...
  return withMetadata as StandardProperty & Record<string, any>;
}

/**
 * Map property type - Real Estate focused
 */
function mapPropertyType(section?: string): string {
 ...
 */
function extractCurrency(priceFormatted?: string): string {
  if (!priceFormatted) return 'CZK';

  if (priceFormatted.includes('€')) return 'EUR';
  if (priceFormatted.includes('Kč')) return 'CZK';
  if (priceFormatted.includes('zł')) return 'PLN';
 ...
 */
function parsePostedDate(from?: string): string | undefined {
 ...
 */
function extractStreet(text: string): string | undefined {
  if (!text) return undefined;
...
export function transformBazosToStandard(
  ...
  country: string = 'cz',
  section?: string,
  llmExtracted?: LLMExtractedProperty
  ...
  const mergedLocation = {
    ...
    postal_code: llmExtracted?.location?.postal_code || (listing as any).detail_zip_code,
  ...
  if (llmExtracted?.details) {
    if (llmExtracted.details.bedrooms !== undefined) details.bedrooms = llmExtracted.details.bedrooms;
    if (llmExtracted.details.bathrooms !== undefined) details.bathrooms = llmExtracted.details.bathrooms;
    if (llmExtracted.details.area_sqm !== undefined) details.area_sqm = llmExtracted.details.area_sqm;
    if (llmExtracted.details.area_total_sqm !== undefined) details.area_total_sqm = llmExtracted.details.area_total_sqm;
    if (llmExtracted.details.area_plot_sqm !== undefined) details.area_plot_sqm = llmExtracted.details.area_plot_sqm;
    if (llmExtracted.details.floor !== undefined) details.floor = llmExtracted.details.floor;
    if (llmExtracted.details.total_floors !== undefined) details.total_floors = llmExtracted.details.total_floors;
    if (llmExtracted.details.rooms !== undefined) details.rooms = llmExtracted.details.rooms;
    if (llmExtracted.details.year_built !== undefined) details.year_built = llmExtracted.details.year_built;
    if (llmExtracted.details.renovation_year !== undefined) details.renovation_year = llmExtracted.details.renovation_year;
    if (llmExtracted.details.parking_spaces !== undefined) details.parking_spaces = llmExtracted.details.parking_spaces;
  }
  ...
  const czechSpecific: Record<string, any> = {};
  ...
  if (llmExtracted?.czech_specific) {
    ...
    if (cs.ownership) czechSpecific.ownership = cs.ownership;
    if (cs.condition) czechSpecific.condition = cs.condition;
    if (cs.furnished) czechSpecific.furnished = cs.furnished;
    if (cs.energy_rating) czechSpecific.energy_rating = cs.energy_rating;
    if (cs.heating_type) czechSpecific.heating_type = cs.heating_type;
    if (cs.construction_type) czechSpecific.construction_type = cs.construction_type;
    if (cs.building_type) czechSpecific.building_type = cs.building_type;
    if (cs.area_balcony !== undefined) czechSpecific.area_balcony = cs.area_balcony;
    if (cs.area_terrace !== undefined) czechSpecific.area_terrace = cs.area_terrace;
    if (cs.area_loggia !== undefined) czechSpecific.area_loggia = cs.area_loggia;
    if (cs.area_cellar !== undefined) czechSpecific.area_cellar = cs.area_cellar;
    if (cs.area_garden !== undefined) czechSpecific.area_garden = cs.area_garden;
    if (cs.water_supply) czechSpecific.water_supply = cs.water_supply;
    if (cs.sewage_type) czechSpecific.sewage_type = cs.sewage_type;
    if (cs.gas_supply !== undefined) czechSpecific.gas_supply = cs.gas_supply;
    if (cs.electricity_supply !== undefined) czechSpecific.electricity_supply = cs.electricity_supply;
    if (cs.rental_period) czechSpecific.rental_period = cs.rental_period;
    if (cs.deposit !== undefined) czechSpecific.deposit = cs.deposit;
    if (cs.monthly_price !== undefined) czechSpecific.monthly_price = cs.monthly_price;
    if (cs.utility_charges !== undefined) czechSpecific.utility_charges = cs.utility_charges;
  }
  ...
  const portalMetadata: Record<string, any> = {
    bazos: {
      ad_id: listing.id,
      section: section,
      country: country,
  ...
  return {
    ...
    images: (listing as any).detail_images?.length > 0
      ? (listing as any).detail_images
      : (listing.image_thumbnail ? [listing.image_thumbnail] : []),
    description: (listing as any).description || '', // From detail page scraping
 ...
 */
function mapLanguageCode(country: string): string {
 ...
 */
export function transformBazosAdsToStandard(
  listings: BazosAd[],
  country: string = 'cz',
  section?: string,
  llmExtractedMap?: Map<string, LLMExtractedProperty>
): StandardProperty[] {
  return listings
    .map(listing => {
      try {
    ...
    .filter((p): p is StandardProperty => p !== null);
}
