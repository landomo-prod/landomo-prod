...
export class MetricsCollector {
  private pool?: Pool;
  private currentMetrics: Partial<ExtractionMetrics> = {};
  private enabled: boolean = false;
  private extractionDurations: number[] = [];
   ...
   */
  public startRun(portal: string, totalListings: number = 0): void {
    this.currentMetrics = {
      timestamp: new Date(),
      portal,
   ...
   */
  public recordExtraction(costUsd: number = 0.000634, durationMs?: number): void {
   ...
   */
  public setTotalListings(total: number): void {
   ...
   */
  public async saveRun(): Promise<boolean> {
    ...
    try {
      ...
      let avgExtractionMs: number | undefined;
   ...
   */
  public async getLatestMetrics(portal: string, limit: number = 1): Promise<ExtractionMetrics[]> {
    if (!this.enabled || !this.pool) {
      return [];
    }
