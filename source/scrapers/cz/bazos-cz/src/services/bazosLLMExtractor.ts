...
import { getAzureOpenAIClient, AzureOpenAIClientManager, ChatMessage } from './azureClient';
import {
  ExtractionResult,
  ExtractionValidation,
  LLMExtractedProperty,
} from '../types/llmExtraction';
import {
  SYSTEM_PROMPT,
  buildUserMessage,
  FEW_SHOT_EXAMPLES,
 ...
 */
export class BazosLLMExtractor {
  private azureClient: AzureOpenAIClientManager;
  private deploymentName: string;

  constructor() {
    this.azureClient = getAzureOpenAIClient();
  ...
  public async extract(
    ...
    try {
      ...
      const truncatedText =
        listingText.length > maxInputLength
      ...
      let extractedData: LLMExtractedProperty;
      try {
   ...
   */
  private buildMessages(listingText: string): ChatMessage[] {
    ...
    messages.push({
      role: 'user',
      content: buildUserMessage(listingText),
    });
   ...
   */
  private validateExtractedData(data: LLMExtractedProperty): ExtractionValidation {
    const warnings: string[] = [];

    // Use schema validation from prompts
    const schemaValidation = validateExtraction(data);
   ...
   */
  private createFallbackData(listingText: string): LLMExtractedProperty {
    return {
      property_type: 'other',
 ...
 */
let extractorInstance: BazosLLMExtractor | null = null;

/**
 * Get singleton LLM extractor
 */
export function getLLMExtractor(): BazosLLMExtractor {
