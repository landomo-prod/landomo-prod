...
// ============================================================================
export const CZECH_DISPOSITIONS = [
...
export type CzechDisposition = typeof CZECH_DISPOSITIONS[number];
...
// ============================================================================
export function normalizeDisposition(input: string | undefined): CzechDisposition | undefined {
  ...
  // Direct matches
  const directMatch = CZECH_DISPOSITIONS.find(d =>
    clean === d.toLowerCase().replace('+', '')
  );
  ...
  if (flatMatch) {
    ...
    if (roomNum >= 1 && roomNum <= 7) return `${flatMatch[1]}+${flatMatch[2].toLowerCase()}` as CzechDisposition;
  }
  ...
  if (dispMatch) {
    ...
    if (roomNum >= 1 && roomNum <= 7) return `${dispMatch[1]}+${dispMatch[2].toLowerCase()}` as CzechDisposition;
  }

  // Ulovdomov camelCase: onepluskk, twoplusone, etc.
  const CAMEL_NUMBERS: Record<string, string> = {
  ...
  if (camelMatch) {
    ...
    if (num) return `${num}+${type}` as CzechDisposition;
  }
  ...
  if (match) {
    ...
    if (roomNum >= 1 && roomNum <= 7) {
      ...
      return `${rooms}+${type}` as CzechDisposition;
    }
  }

  // Special values
  if (clean === 'garsoniera' || clean === 'garsoniéra' || clean === 'garsonka') return '1+kk';
  if (clean === 'sixandmore') return 'atypical';
  if (clean === 'ostatni') return 'atypical';
...
const OWNERSHIP_MAP: Record<string, CzechOwnership> = {
  'Osobní': 'personal',
...
export function normalizeOwnership(input: string | undefined): CzechOwnership | undefined {
  if (!input) return undefined;
  ...
  if (clean.includes('osobn') || clean.includes('soukrom') || clean.includes('personal') || clean.includes('private')) return 'personal';
  if (clean.includes('družstev') || clean.includes('druzstev') || clean.includes('cooperative')) return 'cooperative';
  if (clean.includes('státn') || clean.includes('obecn') || clean.includes('statn') || clean.includes('state')) return 'state';
...
export function normalizeCondition(input: string | undefined): PropertyCondition | undefined {
  ...
  if (clean.includes('novostavb') || clean.includes('nový')) return 'new';
  if (clean.includes('výborn') || clean.includes('excellent')) return 'excellent';
  if (clean.includes('velmi dobr') || clean.includes('very good')) return 'very_good';
  if (clean.includes('dobr') || clean.includes('good')) return 'good';
  if (clean.includes('po rekonstrukc')) return 'after_renovation';
  if (clean.includes('před rekonstrukc')) return 'before_renovation';
  if (clean.includes('nutná rekonstrukc') || clean === 'bad' || clean.includes('špatn') || clean.includes('k demolici')) return 'requires_renovation';
  if (clean.includes('bezvad') || clean.includes('flawless')) return 'excellent';
  if (clean.includes('projekt') || clean.includes('project')) return 'project';
  if (clean.includes('výstavb') || clean.includes('rozestav') || clean.includes('v rekonstrukc')) return 'under_construction';
...
const FURNISHED_MAP: Record<string, FurnishedStatus> = {
  'Ano': 'furnished',
  'Ne': 'not_furnished',
...
const ENERGY_MAP: Record<string, EnergyRating> = {
...
const HEATING_MAP: Record<string, HeatingType> = {
...
export function normalizeConstructionType(input: string | undefined): ConstructionType | undefined {
  ...
  if (clean.includes('skelet') || clean.includes('skeleton')) return 'concrete';
  if (clean.includes('prefab') || clean.includes('montovan')) return 'other';
  if (clean === 'jiná' || clean === 'jina' || clean === 'jiné' || clean === 'jine') return 'other';
...
// VALIDATION HELPERS
// ============================================================================
export function isValidDisposition(value: string): value is CzechDisposition {
  return CZECH_DISPOSITIONS.includes(value as CzechDisposition);
}

export function isValidOwnership(value: string): value is CzechOwnership {
  return CZECH_OWNERSHIP_TYPES.includes(value as CzechOwnership);
}
...
// ============================================================================
// FEATURES ARRAY PARSER - Czech Amenities
// ============================================================================
// Parses Czech feature text strings into structured amenity booleans
// Examples: ["Parkování", "Klimatizace", "Sauna"] → { has_parking: true, has_ac: true, has_sauna: true }
 ...
 */
export function parseCzechFeatures(features?: string[]): ParsedAmenities {
  ...
  // Parking variants
  if (normalized.some(f =>
    f.includes('parkování') || f.includes('parkovani') ||
  ...
  // Garage variants
  if (normalized.some(f =>
    f.includes('garáž') || f.includes('garaz') ||
  ...
  // Balcony variants
  if (normalized.some(f =>
    f.includes('balkon') || f.includes('balcony')
  )) {
  ...
  // Terrace variants
  if (normalized.some(f =>
    f.includes('terasa') || f.includes('terrace') ||
  ...
  if (normalized.some(f =>
    f.includes('sklep') || f.includes('sklepem') ||
  ...
  // Elevator variants
  if (normalized.some(f =>
    f.includes('výtah') || f.includes('vytah') ||
  ...
  if (normalized.some(f =>
    f.includes('logie') || f.includes('logia') ||
  ...
  if (normalized.some(f =>
    f.includes('bezbariér') || f.includes('bezbarier') ||
  ...
  if (normalized.some(f =>
    f.includes('zvířat') || f.includes('zvireata') ||
  ...
  // Garden variants
  if (normalized.some(f =>
    f.includes('zahrada') || f.includes('zahradě') ||
  ...
  if (normalized.some(f =>
    f.includes('nízkoenerget') || f.includes('nizkoenerg') ||
  ...
  if (normalized.some(f =>
    f.includes('fitness') || f.includes('posilovna') ||
  ...
  if (normalized.some(f =>
    f.includes('klimatizace') || f.includes('klimatizaci') ||
  ...
  // WiFi variants
  if (normalized.some(f =>
    f.includes('wifi') || f.includes('wi-fi') ||
    f.includes('internet')
  )) {
  ...
  if (normalized.some(f =>
    f.includes('bezpeč') || f.includes('bezpec') ||
  ...
  // Storage variants
  if (normalized.some(f =>
    f.includes('úložn') || f.includes('ulozn') ||
  ...
  // Hot water variants
  if (normalized.some(f =>
    f.includes('teplá voda') || f.includes('tepla voda') ||
  ...
  // Renovated variants
  if (normalized.some(f =>
    f.includes('rekonstruk') || f.includes('zrekonstruo') ||
