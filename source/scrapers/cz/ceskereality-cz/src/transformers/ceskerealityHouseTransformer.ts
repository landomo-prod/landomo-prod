...
import { normalizeHeatingType, normalizeOwnership, normalizeDisposition, normalizeCondition, normalizeConstructionType, normalizeFurnished } from '../../../shared/czech-value-mappings';
...
function detectHouseSubtype(
  ...
  if (sourceUrl.includes('/chaty-chalupy/')) return 'cottage';

  if (/chalupa|chata|rekreační/.test(text)) return 'cottage';
  if (/vila|villa/.test(text)) return 'villa';
  if (/řadov[ýáé]|řadový dům/.test(text)) return 'terraced';
  if (/dvojdomek|dvoj[gG]eneračn|polovina/.test(text)) return 'semi_detached';
  if (/usedlost|statek|farma|zemědělsk/.test(text)) return 'farmhouse';
  if (/bungalov/.test(text)) return 'bungalow';
  if (/rodinný dům|rodinné domy/.test(text)) return 'detached';
...
export function transformHouse(
  ...
  let sqmLiving: number | undefined;
  let sqmPlot: number | undefined;
  ...
  const property: HousePropertyTierI = {
    ...
    construction_type: normalizeConstructionType(mappedDetails.constructionType) as HousePropertyTierI['construction_type'],
    condition: normalizeCondition(mappedDetails.condition) as HousePropertyTierI['condition'],
