...
exports.detectPropertyCategory = detectPropertyCategory;
exports.getCategoryConfidence = getCategoryConfidence;
exports.testCategoryDetection = testCategoryDetection;
/**
 ...
 */
function detectPropertyCategory(title, description = '') {
 ...
 */
function getCategoryConfidence(title, description = '') {
    const text = (title + ' ' + description).toLowerCase();
    ...
    if (matches >= 2)
        return 1.0; // High confidence (multiple matches)
    if (matches === 1)
        return 0.8; // Medium confidence (single match)
