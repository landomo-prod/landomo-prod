...
exports.transformRealityLand = transformRealityLand;
 ...
 */
function transformRealityLand(listing) {
    ...
    const water_supply = waterBool === true ? 'mains' : waterBool === false ? 'none' : undefined;
    const sewageBool = parseBooleanInfo(info['Kanalizace']);
    const sewage = sewageBool === true ? 'mains' : sewageBool === false ? 'none' : undefined;
    const electricityBool = parseBooleanInfo(info['Elektřina'] || info['Elektrika']);
    const electricity = electricityBool === true ? 'connected' : electricityBool === false ? 'none' : undefined;
    const gasBool = parseBooleanInfo(info['Plyn']);
    const gas = gasBool === true ? 'connected' : gasBool === false ? 'none' : undefined;
    const roadBool = parseBooleanInfo(info['Příjezdová cesta'] || info['Komunikace']);
    const road_access = roadBool === true ? 'paved' : roadBool === false ? 'none' : undefined;
    // Terrain
    ...
    return ({
        ...
        ownership_type: rawOwnership ? (0, czech_value_mappings_1.normalizeOwnership)(rawOwnership) : undefined,
        ...
        country_specific: {
            czech: {
                ownership: rawOwnership ? (0, czech_value_mappings_1.normalizeOwnership)(rawOwnership) : undefined,
                zoning: mapZoning(info['Využití'] || info['Typ pozemku'] || info['Účel pozemku']),
                water_supply,
                sewage,
                electricity,
...
function mapZoning(raw) {
    if (!raw)
        return undefined;
    const v = raw.toLowerCase();
    if (v.includes('bydl') || v.includes('obyt') || v.includes('rezidenční'))
        return 'residential';
    if (v.includes('komerční') || v.includes('obchod'))
        return 'commercial';
    if (v.includes('průmysl') || v.includes('výrob'))
        return 'industrial';
    if (v.includes('zeměděl') || v.includes('orná'))
        return 'agricultural';
    if (v.includes('rekrea') || v.includes('zahrad'))
        return 'recreational';
    if (v.includes('smíšen'))
        return 'mixed';
    return undefined;
}
/** Detect land subtype from zoning/type info */
function detectLandSubtype(typeStr) {
    if (!typeStr)
        return undefined;
    const v = typeStr.toLowerCase();
    if (v.includes('stavební') || v.includes('bydl') || v.includes('rezidenční'))
        return 'building_plot';
    if (v.includes('zeměděl') || v.includes('orná'))
        return 'agricultural';
    if (v.includes('les') || v.includes('lesní'))
        return 'forest';
    if (v.includes('vinic') || v.includes('vinohrad'))
        return 'vineyard';
    if (v.includes('ovocn') || v.includes('sad'))
        return 'orchard';
    if (v.includes('rekrea') || v.includes('zahrad'))
        return 'recreational';
    if (v.includes('průmysl') || v.includes('komerční'))
        return 'industrial';
    return undefined;
}
...
function mapTerrain(raw) {
    if (!raw)
        return undefined;
    const v = raw.toLowerCase();
    if (v.includes('rovný') || v.includes('rovinatý') || v === 'flat')
        return 'flat';
    if (v.includes('svažitý') || v.includes('svazitý') || v.includes('mírně svažitý') || v === 'sloped')
        return 'sloped';
    if (v.includes('členitý') || v.includes('kopcovitý') || v === 'hilly')
        return 'hilly';
    if (v.includes('hornatý') || v === 'mountainous')
        return 'mountainous';
    return undefined;
}
