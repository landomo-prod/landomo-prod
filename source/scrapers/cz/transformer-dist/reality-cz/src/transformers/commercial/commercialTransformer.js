...
exports.transformRealityCommercial = transformRealityCommercial;
 ...
 */
function transformRealityCommercial(listing) {
    ...
    const normalizedCondition = rawCondition ? (0, czech_value_mappings_1.normalizeCondition)(rawCondition) : undefined;
    const condition = mapConditionToTierI(normalizedCondition);
    const rawHeating = info['Topení'] || info['Vytápění'];
    const heating_type = rawHeating ? (0, czech_value_mappings_1.normalizeHeatingType)(rawHeating) : undefined;
    const rawConstruction = info['Stavba'] || info['Druh budovy'] || info['Konstrukce'] || info['Typ budovy'];
    const construction_type = rawConstruction ? (0, czech_value_mappings_1.normalizeConstructionType)(rawConstruction) : undefined;
    const rawEnergy = info['Energetická třída'] || info['PENB'] || info['Energetický štítek'] || info['Energet. náročnost'];
    const energy_class = rawEnergy ? (0, czech_value_mappings_1.normalizeEnergyRating)(rawEnergy) : undefined;
    ...
    const status = listing.outdated ? 'removed' : 'active';
    return ({
        ...
        country_specific: {
            czech: {
                ...
                furnished: (info['Vybavení'] || info['Vybavenost'] || info['Zařízení nábytkem']) ? (0, czech_value_mappings_1.normalizeFurnished)(info['Vybavení'] || info['Vybavenost'] || info['Zařízení nábytkem']) : undefined,
...
// ============ Helper Functions ============
function buildInfoMap(information) {
    const map = {};
...
function parseBooleanInfo(value) {
    if (value === undefined || value === null)
        return undefined;
    const v = value.toLowerCase().trim();
    if (v === '')
        return undefined;
    if (v === 'ne' || v === 'no' || v === '0' || v === 'false' || v === 'není')
        return false;
    if (v === 'ano' || v === 'yes' || v === '1' || v === 'true' || v.startsWith('ano'))
        return true;
    return undefined;
}
/** Extract sqm from api_type string like "obchodní prostory, plocha 1.800 m²" */
function extractSqmFromType(apiType) {
    if (!apiType)
        return undefined;
...
function mapConditionToTierI(normalized) {
    if (!normalized)
        return undefined;
    if (normalized === 'very_good')
        return 'excellent';
    if (normalized === 'before_renovation')
        return 'requires_renovation';
    if (normalized === 'project' || normalized === 'under_construction')
        return 'new';
    return normalized;
}
...
function detectCommercialSubtype(typeStr) {
    if (!typeStr)
        return undefined;
    const t = typeStr.toLowerCase();
    if (t.includes('kancelář') || t.includes('kancelar') || t.includes('office'))
        return 'office';
    if (t.includes('prodej') || t.includes('obchod') || t.includes('retail'))
        return 'retail';
    if (t.includes('sklad') || t.includes('warehouse'))
        return 'warehouse';
    if (t.includes('výrob') || t.includes('průmysl') || t.includes('industrial'))
        return 'industrial';
    if (t.includes('hotel'))
        return 'hotel';
    if (t.includes('restaurace') || t.includes('restaurant'))
        return 'restaurant';
    if (t.includes('ordinace') || t.includes('medical') || t.includes('zdravotní'))
        return 'medical';
    if (t.includes('showroom') || t.includes('výstavní'))
        return 'showroom';
    if (t.includes('smíšen') || t.includes('mixed'))
        return 'mixed_use';
    return undefined;
}
function detectZoning(zoneStr) {
    if (!zoneStr)
        return undefined;
    const z = zoneStr.toLowerCase();
    if (z.includes('komerční') || z.includes('obchod'))
        return 'commercial';
    if (z.includes('průmysl') || z.includes('výrob'))
        return 'industrial';
    if (z.includes('smíšen'))
        return 'mixed_use';
    if (z.includes('kancelář'))
        return 'office';
    if (z.includes('prodej'))
        return 'retail';
    return undefined;
}
function extractFeatures(information) {
