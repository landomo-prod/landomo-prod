...
exports.resetAdaptiveDelay = resetAdaptiveDelay;
exports.fetchAllPages = fetchAllPages;
exports.fetchListingDetail = fetchListingDetail;
...
function _increaseDelay() {
    _adaptiveDelayMs = Math.min(_adaptiveDelayMs === 0 ? 2000 : Math.round(_adaptiveDelayMs * _backoffMultiplier), _backoffMax);
}
...
async function fetchPage(pageUrl, pageNum, propertyType, transactionType) {
    let retries = 5;
    while (retries > 0) {
        try {
            if (_adaptiveDelayMs > 0) {
            ...
            if (!totalCount) {
                ...
                if (metaMatch)
                    totalCount = parseInt(metaMatch[1] || metaMatch[2] || metaMatch[3]);
            }
...
async function fetchAllPages(categoryUrl, transactionType, propertyType, _concurrency = 1, // Sequential only — concurrent page fetching causes pagination drift
onPageFetched, maxPages) {
    const allListings = [];
    ...
    if (!firstResult || firstResult.listings.length === 0)
        return allListings;
    allListings.push(...firstResult.listings);
    if (onPageFetched)
        await onPageFetched(firstResult.listings);
    ...
    for (let pageNum = 2; pageNum <= PAGE_LIMIT; pageNum++) {
        ...
        if (onPageFetched)
            await onPageFetched(result.listings);
        if (!result.hasNext) {
            break;
        }
    }
...
async function fetchListingDetail(url) {
    ...
    $('.b-definition-columns dl dt').each((_, dtEl) => {
        ...
        if (ddEl.hasClass('advertisement'))
            return;
    ...
    $('script:not([src])').each((_, el) => {
        ...
        if (cityMatch && !locationCity)
            locationCity = cityMatch[1];
        const districtMatch = content.match(/"listing_localityDistrict"\s*:\s*"([^"]+)"/);
        if (districtMatch && !locationDistrict)
            locationDistrict = districtMatch[1];
        const regionMatch = content.match(/"listing_localityRegion"\s*:\s*"([^"]+)"/);
        if (regionMatch && !locationRegion)
            locationRegion = regionMatch[1];
        const cityAreaMatch = content.match(/"listing_localityCityArea"\s*:\s*"([^"]+)"/);
        if (cityAreaMatch && !locationCityArea)
            locationCityArea = cityAreaMatch[1];
    });
    ...
    if (!latitude || !longitude) {
        ...
        if (maptilerJson) {
            try {
                ...
                const mainFeature = maptilerData?.geojson?.features?.find((f) => f.properties?.isSimilar === false);
