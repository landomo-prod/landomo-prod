...
exports.transformUlovDomovToStandard = transformUlovDomovToStandard;
...
function transformUlovDomovToStandard(offer) {
    ...
    const energyClass = (0, czech_value_mappings_1.normalizeEnergyRating)(getFirstOptionId(params?.energyEfficiencyRating));
    const condition = (0, czech_value_mappings_1.normalizeCondition)(mapUlovDomovCondition(getFirstOptionId(params?.buildingCondition)));
    const constructionType = (0, czech_value_mappings_1.normalizeConstructionType)(mapUlovDomovMaterial(getFirstOptionId(params?.material)));
    const furnished = (0, czech_value_mappings_1.normalizeFurnished)(mapUlovDomovFurnished(getFirstOptionId(params?.furnished)));
    const ownership = (0, czech_value_mappings_1.normalizeOwnership)(mapUlovDomovOwnership(getFirstOptionId(params?.ownership)));
    const heatingType = (0, czech_value_mappings_1.normalizeHeatingType)(mapUlovDomovHeating(getFirstOptionId(params?.heating)));
    ...
    return {
        ...
        czech_disposition: (0, czech_value_mappings_1.normalizeDisposition)(camelToDisposition(offer.disposition)),
        // Classification
...
function getFirstOptionId(param) {
    return param?.options?.[0]?.id;
}
/** Check if a parameter value is "Ano" (Yes) */
function paramIsYes(value) {
    if (typeof value === 'string')
        return value.toLowerCase() === 'ano';
    return false;
}
...
function parseRenovationYear(value) {
    if (!value)
        return undefined;
    const year = typeof value === 'number' ? value : parseInt(String(value), 10);
...
function mapUlovDomovCondition(id) {
    if (!id)
        return undefined;
    const map = {
        'new': 'new',
...
function mapUlovDomovMaterial(id) {
    if (!id)
        return undefined;
    const map = {
        'brick': 'brick',
...
function mapUlovDomovFurnished(id) {
    if (!id)
        return undefined;
    const map = {
        'complete': 'furnished',
...
function mapUlovDomovOwnership(id) {
    if (!id)
        return undefined;
    const map = {
        'personal': 'personal',
...
function mapUlovDomovHeating(id) {
    if (!id)
        return undefined;
    const map = {
...
function mapElectricityStatus(_params) {
...
function buildFeatures(allConvenience) {
    const features = [];
    const featureMap = {
 ...
 */
function buildCategoryFields(category, offer, allConvenience, detail) {
 ...
 */
function hasConvenience(conveniences, feature) {
 ...
 */
function camelToDisposition(disposition) {
    if (!disposition)
        return undefined;
    const map = {
        'onePlusKk': '1+kk',
 ...
 */
function extractBedrooms(disposition) {
 ...
 */
function classifyFloorLocation(floorLevel, totalFloors) {
    if (floorLevel === null || floorLevel === undefined)
        return undefined;
    if (floorLevel === 0)
        return 'ground_floor';
    if (totalFloors && floorLevel >= totalFloors)
        return 'top_floor';
    return 'middle_floor';
}
/**
 ...
 */
function mapPropertySubtype(category, offer) {
    if (category === 'apartment') {
        if (offer.disposition === 'studio')
            return 'studio';
        if (offer.disposition === 'atelier')
            return 'atelier';
        return undefined;
    }
