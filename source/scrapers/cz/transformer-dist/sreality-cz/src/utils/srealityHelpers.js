...
exports.BUILDING_TYPE_CODES = exports.OWNERSHIP_CODES = exports.DISPOSITION_CODES = void 0;
exports.extractSourceUrl = extractSourceUrl;
exports.bedroomsFromDisposition = bedroomsFromDisposition;
exports.extractDispositionFromTitle = extractDispositionFromTitle;
exports.mapOwnership = mapOwnership;
exports.extractFloorInfo = extractFloorInfo;
exports.mapSubType = mapSubType;
exports.parseArea = parseArea;
exports.findItemValue = findItemValue;
exports.getItemValueAsString = getItemValueAsString;
exports.getStringOrValue = getStringOrValue;
exports.extractCity = extractCity;
exports.isPositiveValue = isPositiveValue;
exports.ensureBoolean = ensureBoolean;
exports.extractHashIdFromUrl = extractHashIdFromUrl;
exports.extractSellerInfo = extractSellerInfo;
exports.extractLabelsFeatures = extractLabelsFeatures;
exports.extractImages = extractImages;
exports.extractVirtualTourUrl = extractVirtualTourUrl;
exports.extractVideoUrl = extractVideoUrl;
exports.extractAreaFromTitle = extractAreaFromTitle;
exports.extractCommissionInfo = extractCommissionInfo;
exports.parseAvailableFrom = parseAvailableFrom;
 ...
 */
function extractSourceUrl(listing, hashId) {
    try {
        ...
        const transaction = TRANSACTION_MAP[categoryType];
        const propertyType = PROPERTY_TYPE_MAP[categoryMain];
 ...
 *   "4+kk" → 3
 */
function bedroomsFromDisposition(disposition) {
    if (!disposition)
        return undefined;
...
function mapOwnership(raw) {
    ...
    if (normalized.includes('osobní') || normalized.includes('ov'))
        return 'personal';
    if (normalized.includes('družstevní') || normalized.includes('db'))
        return 'cooperative';
 ...
 */
function mapSubType(categorySubId) {
 ...
 */
function findItemValue(items, ...fieldNames) {
    if (!items)
        return undefined;
 ...
 */
function extractSellerInfo(embedded) {
    if (!embedded || !embedded.seller)
        return undefined;
 ...
 */
function extractLabelsFeatures(labelsAll) {
 ...
 */
function resolveImageUrl(href, width, height) {
    if (!href)
        return undefined;
...
function extractImages(listing) {
    ...
    if (listing._embedded?.images && Array.isArray(listing._embedded.images)) {
        const sorted = [...listing._embedded.images].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
 ...
 * ```
 */
function extractVirtualTourUrl(listing) {
 ...
 */
function extractVideoUrl(listing) {
 ...
 */
function extractCommissionInfo(priceNote) {
    if (!priceNote)
        return {};
