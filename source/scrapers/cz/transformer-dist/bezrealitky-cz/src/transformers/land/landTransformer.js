...
exports.transformBezrealitkyLand = transformBezrealitkyLand;
 ...
 */
function transformBezrealitkyLand(listing) {
    // ============ Core Identification ============
    ...
    const waterPos = listing.waterPipePos; // Position field (GraphQL may have this)
    let water_supply;
    ...
    const sewagePos = listing.sewagePipePos; // Position field (GraphQL may have this)
    let sewage;
    ...
    let electricity;
    const electricityField = listing.electricity;
    ...
    const cadastral_number = listing.cadastralNumber || listing.cadastralArea;
    // Ownership type
 ...
 */
function extractLandFeatures(listing) {
    const features = [];
    ...
    if (listing.water)
        features.push('water_available');
    if (listing.sewage)
        features.push('sewage_available');
    if (listing.electricity)
        features.push('electricity_available');
    if (listing.gas)
        features.push('gas_available');
    // Media
    if (listing.tour360)
        features.push('virtual_tour');
    // Additional metadata
    if (listing.cadastralNumber || listing.cadastralArea) {
