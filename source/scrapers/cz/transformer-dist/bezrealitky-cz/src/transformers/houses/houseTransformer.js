...
exports.transformBezrealitkyHouse = transformBezrealitkyHouse;
 ...
 */
function transformBezrealitkyHouse(listing) {
    // ============ Core Identification ============
    ...
    const construction_type = constructionTypeRaw === 'panel' || constructionTypeRaw === 'other'
        ? undefined
        : constructionTypeRaw;
 ...
 */
function detectHouseSubtype(listing) {
 ...
 */
function extractHouseFeatures(listing) {
    const features = [];
    // Amenities
    if (listing.terrace)
    ...
    if (listing.sewage)
        features.push('sewage_connection');
    return features;
}
