...
exports.transformBezrealitkyApartment = transformBezrealitkyApartment;
...
const bezrealitkyHelpers_2 = require("../../utils/bezrealitkyHelpers");
/**
 ...
 */
function transformBezrealitkyApartment(listing) {
    // ============ Core Identification ============
    ...
    const bedrooms = (0, bezrealitkyHelpers_1.bedroomsFromDisposition)(listing.disposition || '');
    // ============ Apartment-Specific Details ============
    const sqm = listing.surface ?? 0;
    const floor = listing.floor ? (0, bezrealitkyHelpers_1.parseFloor)(listing.floor) : undefined;
    const total_floors = listing.totalFloors;
    // Calculate total rooms (Czech: 2+kk = 2 rooms)
    const rooms = extractRooms((0, bezrealitkyHelpers_1.normalizeBezrealitkyDisposition)(listing.disposition || ''));
    ...
    const constructionTypeRaw = (0, czech_value_mappings_1.normalizeConstructionType)(listing.construction);
    const construction_type = constructionTypeRaw === 'other' || constructionTypeRaw === 'stone' || constructionTypeRaw === 'wood'
        ? undefined
        : constructionTypeRaw;
    const conditionRaw = (0, czech_value_mappings_1.normalizeCondition)(listing.condition);
    const condition = conditionRaw === 'very_good' || conditionRaw === 'before_renovation' || conditionRaw === 'project' || conditionRaw === 'under_construction'
        ? 'good'
        : conditionRaw;
    ...
    const floorLocationRaw = (0, bezrealitkyHelpers_1.extractFloorLocation)(listing.floor, listing.totalFloors);
    const floor_location = floorLocationRaw === 'semi_basement' ? 'ground_floor' : floorLocationRaw;
    // ============ Tier 1 Universal Fields ============
    const furnished = (0, czech_value_mappings_1.normalizeFurnished)(listing.equipped);
    const renovation_year = (0, bezrealitkyHelpers_2.parseRenovationYear)(listing.reconstruction);
    ...
    const media = {
        images: richImages.length > 0 ? richImages : [],
    ...
    return {
        ...
        country_specific: {
            czech_disposition: (0, czech_value_mappings_1.normalizeDisposition)((0, bezrealitkyHelpers_1.normalizeBezrealitkyDisposition)(listing.disposition || '')),
            czech_ownership: listing.ownership ? (0, bezrealitkyHelpers_1.normalizeOwnership)(listing.ownership) : undefined,
 ...
 */
function detectPropertySubtype(listing) {
...
function extractFeatures(listing) {
    ...
    // Building features
    if (listing.newBuilding)
        features.push('new_building');
    if (listing.lowEnergy)
        features.push('low_energy');
    // Living conditions
    if (listing.petFriendly)
        features.push('pet_friendly');
    if (listing.barrierFree)
        features.push('barrier_free');
    if (listing.equipped === 'EQUIPPED')
        features.push('furnished');
    // Media
    if (listing.tour360)
        features.push('virtual_tour');
    // Short-term rental
    if (listing.shortTerm)
        features.push('short_term_rental');
    return features;
}
