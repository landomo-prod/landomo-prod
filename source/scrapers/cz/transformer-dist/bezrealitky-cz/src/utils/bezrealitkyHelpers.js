...
exports.normalizeBezrealitkyDisposition = normalizeBezrealitkyDisposition;
...
exports.parseRenovationYear = parseRenovationYear;
/**
 ...
 */
function normalizeBezrealitkyDisposition(disposition) {
    if (!disposition)
        return '';
...
function normalizeOwnership(ownership) {
    ...
    if (norm.includes('osobni') || norm === 'ov')
        return 'personal';
    if (norm.includes('druzstevni') || norm === 'db')
        return 'cooperative';
    if (norm.includes('statni'))
        return 'state';
    if (norm.includes('obecni') || norm.includes('mestske'))
        return 'municipal';
 ...
 */
function extractFloorLocation(floor, totalFloors) {
    if (!floor)
        return undefined;
    const clean = floor.toLowerCase().trim();
    // Ground floor
 ...
 * - "ano" / "ne" → undefined
 */
function parseRenovationYear(reconstruction) {
    if (!reconstruction)
        return undefined;
