...
exports.mapPropertyDetails = mapPropertyDetails;
/**
 ...
 */
function mapConstructionType(value) {
    if (!value)
        return undefined;
    const lower = value.toLowerCase();
    if (lower.includes('panel'))
        return 'panel';
    if (lower.includes('cihla') || lower.includes('cihelná') || lower.includes('cihlová') || lower.includes('zděná'))
        return 'brick';
    if (lower.includes('beton') || lower.includes('železobeton'))
        return 'concrete';
    if (lower.includes('skelet') || lower.includes('ocel'))
        return 'concrete'; // Skeleton/steel frame → concrete
    if (lower.includes('smíšen') || lower.includes('kombinovan') || lower.includes('jiná') || lower.includes('jiné') || lower.includes('ostatní'))
        return 'mixed';
    return undefined;
}
/**
...
function mapCondition(value) {
    if (!value)
        return undefined;
    const lower = value.toLowerCase();
    if (lower.includes('novostavba') || lower.includes('nový') || lower.includes('nová'))
        return 'new';
    ...
    if (lower.includes('po rekonstrukci') || lower.includes('po renovaci') || lower.includes('po rekonstrukc'))
        return 'after_renovation';
    if (lower.includes('výborný') || lower.includes('vynikající') || lower.includes('velmi dobrý') || lower.includes('bezvadný'))
        return 'excellent';
    if (lower.includes('dobrý') || lower.includes('udržovaný') || lower.includes('zachovalý'))
        return 'good';
    return undefined;
}
/**
 ...
 */
function mapFurnished(value) {
    if (!value)
        return undefined;
    const lower = value.toLowerCase();
    if (lower.includes('částečně') || lower.includes('částečné'))
        return 'partially_furnished';
    if (lower.includes('ano') || lower.includes('vybavený') || lower.includes('zařízený'))
        return 'furnished';
    if (lower.includes('ne') || lower.includes('nevybavený') || lower.includes('nezařízený'))
 ...
 */
function mapPropertyDetails(details) {
    const mapped = {};
    ...
    for (const [label, value] of Object.entries(details)) {
        ...
        if (lower === 'balkóny' || lower === 'balkony') {
            const valueLower = value.toLowerCase();
            if (/balkón|balkon/.test(valueLower))
                mapped.hasBalcony = true;
            if (/terasa|terrace/.test(valueLower))
                mapped.hasTerrace = true;
            if (/lodžie|loggie/.test(valueLower))
                mapped.hasLoggia = true;
        }
    }
