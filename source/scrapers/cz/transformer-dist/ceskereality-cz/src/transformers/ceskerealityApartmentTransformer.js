...
function parseEnergyFromHtml(energyRating) {
    if (!energyRating)
        return undefined;
 ...
 */
function detectApartmentSubtype(title, description) {
    const text = `${title || ''} ${description || ''}`.toLowerCase();
    if (/penthouse|penthaus/.test(text))
        return 'penthouse';
    if (/loft/.test(text))
        return 'loft';
    if (/ateli[eé]r/.test(text))
        return 'atelier';
    if (/mezonet|maisonette|maisonet/.test(text))
        return 'maisonette';
    if (/garsoni[eé]r|garson[ií]|studio|1\+kk/.test(text))
        return 'studio';
    return undefined;
}
function transformApartment(jsonLd, sourceUrl, htmlData) {
