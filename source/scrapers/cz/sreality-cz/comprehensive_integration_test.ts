...
/**
 * Comprehensive Integration Test for SReality Scraper
 * Tests all new features: seller info, accessibility, virtual tours, built area, and areas
 * Uses both list endpoint (for discovery) and detail endpoint (for full data with items)
 */

interface TestResult {
  listing_hash_id: number | string | undefined;
  listing_url: string;
  transformation_success: boolean;
 ...
 */
async function fetchListingDetail(hashId: number, userAgent: string): Promise<SRealityListing | null> {
  try {
 ...
 */
async function fetchRandomListings(count: number = 5): Promise<SRealityListing[]> {
  ...
  try {
    ...
    for (const page of pages) {
      try {
        ...
        allHashIds.push(...estates.map((e: any) => e.hash_id));
 ...
 */
function testTransformer(listing: SRealityListing): TestResult {
  ...
  try {
    // Transform listing
    const standardProperty = transformSRealityToStandard(listing);
 ...
 */
function calculateCoverageMetrics(results: TestResult[]): CoverageMetrics {
  ...
  for (const field of fields) {
    const count = results.filter(r => {
      const value = (r.extracted_fields as any)[field];
 ...
 */
async function runIntegrationTest(): Promise<IntegrationTestReport> {
