...
import { normalizeCondition, normalizeConstructionType, normalizeFurnished, normalizeEnergyRating, normalizeHeatingType } from '../../../../shared/czech-value-mappings';
import type { PropertyCondition, ConstructionType } from '../../../../shared/czech-value-mappings';
 ...
 */
function mapConditionToCommercial(
 ...
 */
function mapConstructionTypeToCommercial(
  constructionType: ConstructionType | undefined
): 'brick' | 'concrete' | 'steel' | 'mixed' | 'prefab' | undefined {
 ...
 */
export function transformBezrealitkyCommercial(
  ...
  const rawCondition = normalizeCondition(listing.condition);
  ...
  const rawConstructionType = normalizeConstructionType(listing.construction);
  ...
  // ============ Media ============
  const richImages: PropertyImage[] = (listing.publicImages || []).map((img: any) => ({
    url: img.url,
    order: img.order,
  ...
  return {
    ...
    // Portal & lifecycle
    source_url: listing.uri ? `https://www.bezrealitky.cz${listing.uri}` : `https://www.bezrealitky.cz/nemovitosti/${listing.id}`,
    source_platform: 'bezrealitky',
    portal_id: `bezrealitky-${listing.id}`,
    status: (listing.active ? 'active' : 'removed') as 'active' | 'removed' | 'sold' | 'rented',
  };
}
