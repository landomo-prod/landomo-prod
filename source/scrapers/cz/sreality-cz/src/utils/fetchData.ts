...
/**
 * Result type for estate detail fetch
 */
 ...
 */
export const fetchEstateDetail = async (hash_id: number, headers: Record<string, string>): Promise<EstateDetailResult> => {
...
async function fetchPage(
  ...
  perPage: number = 100,
  sort?: number
): Promise<{ estates: any[], totalResults: number }> {
...
export const fetchAllListingPages = async (
  ...
  categoryType: number,
  maxPages?: number,
  onBatchFetched?: (listings: any[]) => Promise<void>,
  sort?: number,
): Promise<any[]> => {
  ...
  while (hasMore && (!maxPages || currentPage <= maxPages)) {
    ...
    const pageResults = await Promise.allSettled(
      pageNumbers.map(pageNum => fetchPage(pageNum, category, categoryType, 100, sort))
    );
