...
const PORTAL = 'idnes-reality';

const log = createLogger({ service: 'idnes-reality-scraper', portal: PORTAL, country: 'czech_republic' });

// Start detail workers (3 concurrent by default - rate limit safe)
...
/**
 * Main scraper logic - Three-Phase Queue-Based Architecture
 */
async function runScraper(categories?: string[]) {
  ...
  log.info({ workers: WORKER_CONCURRENCY, categories: categories || 'all' }, 'Starting three-phase checksum scrape');
  ...
  try {
    ...
    while (true) {
      ...
      if (now - lastLog >= 10000) {
        log.info({ waiting: stats.waiting, active: stats.active, failed: stats.failed }, 'Queue progress');
        lastLog = now;
      }
      ...
      if (now >= drainDeadline) {
        log.warn({ waiting: stats.waiting, active: stats.active }, `Full scan drain timed out after ${FULL_SCAN_TIMEOUT_MS}ms`);
        break;
      }
    ...
    scraperMetrics.propertiesScraped.inc({ portal: PORTAL, category: 'all', result: 'success' }, phaseStats.phase1.totalListings);
    scraperMetrics.scrapeRuns.inc({ portal: PORTAL, status: 'success' });
    scraperMetrics.scrapeRunActive.set({ portal: PORTAL }, 0);

    log.info({
      durationSec: durationSec.toFixed(2),
      total: phaseStats.phase1.totalListings,
      queued: phaseStats.phase3.queued,
      new: phaseStats.phase2.new,
      changed: phaseStats.phase2.changed,
      unchanged: phaseStats.phase2.unchanged,
...
app.listen(PORT, '0.0.0.0', () => {
  log.info({ port: PORT, workers: WORKER_CONCURRENCY }, 'Reality.idnes.cz scraper running (queue mode)');
});
