import { ApartmentPropertyTierI, PropertyLocation, PropertyAgent } from '@landomo/core';
 ...
 */
export function transformIdnesApartment(listing: IdnesListing): ApartmentPropertyTierI {
  // ============ Core Identification ============
  ...
  const floor = listing.floor ?? parseFloorFromAttrs(listing._attributes);
  ...
  const balcony_area = parseAreaFromAttrs(listing._attributes, ['plocha balkonu', 'balkón', 'balkon']);
  const loggia_area = parseAreaFromAttrs(listing._attributes, ['plocha lodžie', 'lodžie']);
  const terrace_area = parseAreaFromAttrs(listing._attributes, ['plocha terasy', 'terasa']);
  const cellar_area = parseAreaFromAttrs(listing._attributes, ['plocha sklepa', 'sklep']);
  ...
  const has_garage = amenities.has_garage ?? checkGarageFromParking(listing._attributes);
  ...
  const condition = normalizedCondition === 'very_good' ? 'excellent' :
                    ...
                    (normalizedCondition as ApartmentPropertyTierI['condition']);
  const heating_type = listing.heatingType ? normalizeHeatingType(listing.heatingType) : undefined;
  const construction_type = listing.constructionType ? normalizeConstructionType(listing.constructionType) as ApartmentPropertyTierI['construction_type'] : undefined;
  ...
  const furnished = listing.furnished ? normalizeFurnished(listing.furnished) as ApartmentPropertyTierI['furnished'] : undefined;
  ...
  return ({
    ...
    country_specific: {
      czech_disposition: normalizeDisposition(listing.rooms || listing._attributes?.['počet místností'] || extractDispositionFromTitle(listing.title)),
    ...
    has_parking,
    parking_spaces,
    has_loggia: amenities.has_loggia ?? parseBooleanFromAttr(listing._attributes?.['lodžie']) ?? (loggia_area !== undefined ? true : undefined),
    loggia_area,
