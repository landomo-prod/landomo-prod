    ...
    try {
      ...
      const response = await axios.post(
        ...
        payload,
        {
          ...
          timeout: 60000 // 60 second timeout (increased due to high ingest load)
        }
      );

      console.log(JSON.stringify({ level: 'info', service: 'realingo-scraper', msg: 'Batch sent to ingest', count: properties.length }));

      return response.data;
    } catch (error: any) {
      console.error(JSON.stringify({ level: 'error', service: 'realingo-scraper', msg: 'Failed to send batch', count: properties.length, err: error.message, status: error.response?.status }));
