...
function buildHouseFeatures(detail?: RealingoOffer['detail']): string[] {
...
export function transformRealingoHouse(offer: RealingoOffer): HousePropertyTierI {
  ...
  const sqm_living = (offer.area?.main ?? offer.area?.floor ?? offer.area?.built ?? null) as number;
  const sqm_plot = (offer.area?.plot ?? null) as number;
  const bedrooms = (detail?.roomCount ? Math.max(detail.roomCount - 1, 1) : null) as number;
  ...
  return {
    ...
    transaction_type,
    location,
    property_subtype: (detail?.houseType?.toLowerCase() ?? undefined) as any,
    bedrooms,
    bathrooms: 1,
    sqm_living,
