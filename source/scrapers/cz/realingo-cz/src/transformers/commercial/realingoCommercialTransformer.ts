...
function mapCommercialSubtype(category?: string): CommercialPropertyTierI['property_subtype'] {
  switch (category) {
...
function buildCommercialFeatures(detail?: RealingoOffer['detail']): string[] {
...
export function transformRealingoCommercial(offer: RealingoOffer): CommercialPropertyTierI {
  ...
  return {
    ...
    property_subtype: classifyCommercialPropertyType(title) as any,
    title,
    price,
    currency,
    transaction_type,
    location,
    sqm_total: (offer.area?.main ?? offer.area?.floor ?? offer.area?.built ?? null) as number,
...
function classifyFloorLocation(floor?: number | null, totalFloors?: number | null): CommercialPropertyTierI['floor_location'] {
