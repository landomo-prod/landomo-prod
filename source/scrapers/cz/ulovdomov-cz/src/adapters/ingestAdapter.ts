...
export class IngestAdapter {
  ...
  async sendProperties(properties: PropertyPayload[], scrapeRunId?: string): Promise<void> {
    ...
    try {
      ...
      console.log(JSON.stringify({ level: 'info', service: 'ulovdomov-scraper', msg: 'Batch sent to ingest', count: properties.length }));

      return response.data;
    } catch (error: any) {
      console.error(JSON.stringify({ level: 'error', service: 'ulovdomov-scraper', msg: 'Failed to send batch', count: properties.length, err: error.message, status: error.response?.status }));
