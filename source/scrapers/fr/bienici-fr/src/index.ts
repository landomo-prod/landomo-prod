...
const PORTAL = 'bienici';

const log = createLogger({ service: 'bienici-scraper', portal: PORTAL, country: 'france' });
...
async function runScraper() {
  ...
  log.info({ workers: WORKER_CONCURRENCY }, 'Starting three-phase price-band scrape');
...
app.listen(PORT, '0.0.0.0', () => {
  log.info({ port: PORT, workers: WORKER_CONCURRENCY }, 'Bienici scraper running');
});
