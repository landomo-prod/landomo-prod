...
export function transformHouse(listing: EdcListingRaw): HousePropertyTierI {
  ...
  // Danish 'Sommerhus' (holiday cottage) → mark as summer house subtype
  const isSommerHus = listing.estateTypeName?.toLowerCase().includes('sommerhus') ?? false;
  const isLandejendom = listing.estateTypeName?.toLowerCase().includes('landejendom') ?? false;
  ...
  return {
    ...
    property_subtype: isSommerHus ? 'cottage' : isLandejendom ? 'farmhouse' : undefined,
    location: {
    ...
    bedrooms,
    sqm_living: listing.livingArea?.value ?? 0,
    sqm_plot: listing.areaLand?.value ?? 0,
    rooms: rooms > 0 ? Math.floor(rooms) : undefined,

    // EDC search endpoint does not return amenity flags
    has_garden: (listing.areaLand?.value ?? 0) > 0,
    has_garage: false,
    has_parking: false,
    has_basement: false,
