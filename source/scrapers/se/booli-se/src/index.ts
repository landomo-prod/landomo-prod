...
import { transformBooliListing } from './transformers';
import { BooliListingShort, TransactionType } from './types/booliTypes';

const app = express();
const PORT = parseInt(process.env.PORT || '8221', 10);
const PORTAL = 'booli-se';
const COUNTRY = 'sweden';

const log = createLogger({ service: 'booli-scraper', portal: PORTAL, country: COUNTRY });
...
async function runScraper(): Promise<void> {
  ...
  log.info('Starting Booli scrape');
  ...
  try {
    ...
    const handleBatch = async (
      ...
      const properties = batch
        .map(listing => {
          try {
            const transformed = transformBooliListing(listing, transactionType);
            const payload: PropertyPayload = {
              portalId: `booli-${listing.booliId}`,
              data: transformed,
              rawData: listing,
            };
            return payload;
            ...
            log.error(
              { booliId: listing.booliId, objectType: listing.objectType, err: error },
              'Error transforming listing'
            );
            return null;
          }
        })
        .filter((p): p is PropertyPayload => p !== null);
...
app.listen(PORT, '0.0.0.0', () => {
  log.info({ port: PORT }, 'Booli scraper running');
});
