...
export class IngestAdapter {
  ...
  async sendProperties(properties: PropertyPayload[]): Promise<void> {
    ...
    try {
      ...
      console.log(JSON.stringify({ level: 'info', service: 'immoscout24-ch', msg: 'Sent batch', count: properties.length }));
    } catch (error: any) {
      console.error(JSON.stringify({ level: 'error', service: 'immoscout24-ch', msg: 'Failed to send batch', err: error.message }));
      throw error;
    }
  }
}
