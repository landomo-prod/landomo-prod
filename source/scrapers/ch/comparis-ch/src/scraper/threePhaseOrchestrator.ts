...
import { batchCreateComparisChecksums } from '../utils/checksumExtractor';
import { addDetailJobs } from '../queue/detailQueue';

// Comparis categories: apartment, house, land, commercial
// Deal types: buy, rent
const CATEGORIES = [
  { propertyType: 'apartment', dealType: 'buy' as const },
  { propertyType: 'apartment', dealType: 'rent' as const },
  { propertyType: 'house', dealType: 'buy' as const },
  { propertyType: 'house', dealType: 'rent' as const },
  { propertyType: 'land', dealType: 'buy' as const },
  { propertyType: 'commercial', dealType: 'buy' as const },
  { propertyType: 'commercial', dealType: 'rent' as const },
];
...
export async function runThreePhaseScrape(scrapeRunId?: string): Promise<PhaseStats> {
  ...
  for (const { propertyType, dealType } of CATEGORIES) {
    console.log(JSON.stringify({ level: 'info', service: 'comparis-ch', msg: 'Fetching category', propertyType, dealType }));
    const listings = await fetchAllListingPages(dealType, propertyType);
    stats.phase1.categoriesProcessed++;
    stats.phase1.totalListings += listings.length;
    console.log(JSON.stringify({ level: 'info', service: 'comparis-ch', msg: 'Category fetched', propertyType, dealType, count: listings.length }));

    if (listings.length === 0) continue;

    // Phase 2: Compare checksums
    const phase2Start = Date.now();
    const checksums = batchCreateComparisChecksums(listings);
    ...
    const comparison = await checksumClient.compareChecksumsInBatches(
      ...
      5000,
      (current, total) => {
        console.log(JSON.stringify({ level: 'info', service: 'comparis-ch', msg: 'Checksum progress', propertyType, dealType, current, total }));
      }
    );
    ...
    try {
      ...
      console.error(JSON.stringify({ level: 'error', service: 'comparis-ch', msg: 'Failed to store checksums', propertyType, dealType, err: error.message }));
    }
    ...
    const jobs = listings
      .filter(listing => {
        const id = listing.id?.toString() ?? listing.adId?.toString();
        return id && toFetchSet.has(id);
      })
      .map(listing => ({
        listingId: (listing.id ?? listing.adId).toString(),
        propertyType,
        dealType,
        listingData: listing,
      }));
    ...
    console.log(JSON.stringify({ level: 'info', service: 'comparis-ch', msg: 'Category processed', propertyType, dealType, new: comparison.new, changed: comparison.changed, unchanged: comparison.unchanged, queued: jobs.length }));
  }
