...
export class IngestAdapter {
  ...
  async sendProperties(properties: PropertyPayload[]): Promise<void> {
    ...
    try {
      ...
      console.log(JSON.stringify({ level: 'info', service: this.portal, msg: 'Batch sent to ingest', count: properties.length }));
      return response.data;
    } catch (error: any) {
      console.error(JSON.stringify({ level: 'error', service: this.portal, msg: 'Failed to send batch', count: properties.length, err: error.message, status: error.response?.status }));
      throw error;
    }
  }
