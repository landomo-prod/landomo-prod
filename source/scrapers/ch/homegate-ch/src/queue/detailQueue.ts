...
export const detailQueue = new Queue('homegate-ch-details', {
...
const adapter = new IngestAdapter('homegate-ch');
...
async function flushBatch() {
  ...
  try {
    ...
    batch = [];
    console.log(JSON.stringify({ level: 'info', service: 'homegate-ch', msg: 'Sent batch', count: batchSize }));
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', service: 'homegate-ch', msg: 'Failed to send batch', err: (error as any)?.message || String(error) }));
  }
}
...
export function createDetailWorker(concurrency: number = 3) {
  const worker = new Worker<DetailJob>(
    ...
    async (job: Job<DetailJob>) => {
      const { propertyId, category, offerType } = job.data;

      try {
        await new Promise(resolve => setTimeout(resolve, 500 + Math.random() * 1000));

        const detailData = await fetchPropertyDetail(propertyId, offerType);
        if (!detailData) {
        ...
        batch.push({
          portalId: `homegate-ch-${propertyId}`,
          data: standardData,
          rawData: detailData,
        });
        ...
        console.error(JSON.stringify({ level: 'error', service: 'homegate-ch', msg: 'Failed to process property', propertyId, err: error.message }));
        throw error;
      }
    },
    {
  ...
  worker.on('failed', (job: any, err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'homegate-ch', msg: 'Job failed', propertyId: job?.data.propertyId, err: err.message }));
  });

  worker.on('error', (err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'homegate-ch', msg: 'Worker error', err: err?.message || String(err) }));
  });
...
export async function addDetailJobs(jobs: DetailJob[]) {
  ...
  console.log(JSON.stringify({ level: 'info', service: 'homegate-ch', msg: 'Queued detail jobs', count: jobs.length }));
}
