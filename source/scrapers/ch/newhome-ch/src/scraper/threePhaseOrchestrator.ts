...
import { batchCreateNewhomeChecksums } from '../utils/checksumExtractor';
import { addDetailJobs } from '../queue/detailQueue';

const CATEGORIES = [
  { propertyType: 'apartment', offerType: 'buy' as const },
  { propertyType: 'apartment', offerType: 'rent' as const },
  { propertyType: 'house', offerType: 'buy' as const },
  { propertyType: 'house', offerType: 'rent' as const },
  { propertyType: 'land', offerType: 'buy' as const },
  { propertyType: 'commercial', offerType: 'buy' as const },
  { propertyType: 'commercial', offerType: 'rent' as const },
];
...
export async function runThreePhaseScrape(scrapeRunId?: string): Promise<PhaseStats> {
  ...
  for (const { propertyType, offerType } of CATEGORIES) {
    console.log(JSON.stringify({ level: 'info', service: 'newhome-ch', msg: 'Fetching category', propertyType, offerType }));
    const listings = await fetchAllListingPages(offerType, propertyType);
    ...
    const checksums = batchCreateNewhomeChecksums(listings);
    ...
    const comparison = await checksumClient.compareChecksumsInBatches(
      ...
      5000,
      (current, total) => {
        console.log(JSON.stringify({ level: 'info', service: 'newhome-ch', msg: 'Checksum progress', propertyType, offerType, current, total }));
      }
    );
    ...
    try {
      ...
      console.error(JSON.stringify({ level: 'error', service: 'newhome-ch', msg: 'Failed to store checksums', err: error.message }));
    }
    ...
    const jobs = listings
      .filter(listing => {
        const id = listing.id?.toString() ?? listing.objectId?.toString();
        return id && toFetchSet.has(id);
      })
      .map(listing => ({
        listingId: (listing.id ?? listing.objectId).toString(),
        propertyType,
        offerType,
        listingData: listing,
      }));
    ...
    console.log(JSON.stringify({ level: 'info', service: 'newhome-ch', msg: 'Category processed', propertyType, offerType, new: comparison.new, changed: comparison.changed, unchanged: comparison.unchanged, queued: jobs.length }));
  }
