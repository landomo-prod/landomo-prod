/**
 * User agent pool for Immowelt Germany scraper
...
/**
 * Mobile user agents for mobile app simulation
 */
...
/**
 * Get a desktop user agent (default)
 */
...
/**
 * Get Immowelt-specific headers with rotation
 */
export function getImmoweltHeaders(): Record<string, string> {
  ...
  if (Math.random() > 0.5) {
    headers['Referer'] = 'https://www.immowelt.de/';
  }
...
/**
 * Get a consistent user agent for the entire session
 * Uses environment variable or picks a default Chrome UA
 */
export function getSessionUserAgent(): string {
  return process.env.USER_AGENT || USER_AGENTS[0];
}
