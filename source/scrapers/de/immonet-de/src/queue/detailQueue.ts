...
import { transformImmonetToStandard } from '../transformers/immonetTransformer';
import { IngestAdapter, PropertyPayload } from '../adapters/ingestAdapter';
import { createLogger } from '@landomo/core';

const log = createLogger({ service: 'immonet-de-scraper', portal: 'immonet-de', country: 'de' });

// Config from env
const WORKER_CONCURRENCY = parseInt(process.env.WORKER_CONCURRENCY || '3');
const RATE_LIMIT_MS = parseInt(process.env.DETAIL_RATE_LIMIT_MS || '2000');
const BATCH_SIZE = parseInt(process.env.BATCH_SIZE || '50');

const QUEUE_NAME = 'immonet-de-details';
...
const adapter = new IngestAdapter('immonet-de');
...
async function flushBatch() {
  ...
  try {
    ...
    // Re-add to batch for next flush attempt
    batch.unshift(...toSend);
  }
}

// Shared scraper instance for detail fetches (reuses browser)
  ...
  const worker = new Worker<DetailJob>(
    ...
    async (job: Job<DetailJob>) => {
      ...
      try {
        ...
        // Fetch detail page using existing Playwright logic
        ...
        // Merge detail data with basic info
        const mergedListing = {
          id,
          url,
          transactionType,
        ...
        const standardData = transformImmonetToStandard(mergedListing as any);
