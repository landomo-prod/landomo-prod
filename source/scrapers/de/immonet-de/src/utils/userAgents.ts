/**
 * User agent pool for Immonet Germany scraper
 * Expanded with realistic browser fingerprints to avoid detection
 */

/**
 * Browser user agents - realistic desktop browsers (20+ variations)
 */
...
/**
 * Get default user agent (Chrome on macOS)
 */
export function getDefaultUserAgent(): string {
  return userAgents[5]; // Chrome on macOS
}

/**
 * Get Immonet-specific headers with rotation
 */
export function getImmonetHeaders(): Record<string, string> {
  ...
  if (Math.random() > 0.5) {
    headers['Referer'] = 'https://www.immonet.de/';
  }
