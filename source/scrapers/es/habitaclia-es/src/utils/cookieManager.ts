...
const HABITACLIA_URL = 'https://www.habitaclia.com/';
...
export async function getCFCookies(): Promise<string> {
  ...
  refreshPromise = (async () => {
    console.log(JSON.stringify({ level: 'info', service: 'habitaclia-scraper', msg: 'Launching Playwright to get CF cookies' }));
    ...
    try {
      ...
      await page.goto(HABITACLIA_URL, { waitUntil: 'networkidle', timeout: 30000 });

      // Navigate to a real listing page to get all cookies
      await page.goto('https://www.habitaclia.com/pisos-barcelona.htm', { waitUntil: 'networkidle', timeout: 30000 }).catch(() => {});
