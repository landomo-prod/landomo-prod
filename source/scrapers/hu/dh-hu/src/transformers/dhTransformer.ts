...
import { DHListing, PROPERTY_TYPE_MAP, TRANSACTION_TYPE_MAP } from '../types/dhTypes';

/**
 * Transform Duna House (dh.hu) listing to StandardProperty format
 */
export function transformDHToStandard(listing: DHListing): StandardProperty & Record<string, any> {
  const sqm = listing.area;

  return {
    // Basic info
    title: listing.title || 'Untitled Property',
    price: listing.price || listing.priceNumeric || 0,
    currency: listing.currency || 'HUF',
    property_type: mapPropertyType(listing.propertyType || listing.propertyTypeName || ''),
    property_category: mapPropertyCategory(mapPropertyType(listing.propertyType || listing.propertyTypeName || '')),
    transaction_type: mapTransactionType(listing.transactionType),
    source_url: listing.url,
    source_platform: 'dh.hu',

    // Agent/Seller information
    agent: listing.agent ? {
      name: listing.agent.name || listing.agent.company || 'Duna House',
      phone: listing.agent.phone,
      email: listing.agent.email,
      agency: listing.agent.company || 'Duna House'
    } : {
      name: 'Duna House',
      agency: 'Duna House'
    },
    ...
    details: {
      ...
      renovation_year: undefined, // DH.hu doesn't provide renovation year separately
    ...
    available_from: undefined, // DH.hu doesn't provide available_from
    ...
    portal_metadata: {
      'dh.hu': {
        ...
        is_exclusive: listing.isExclusive,
        enabled_otthon_start: listing.enabledOtthonStart
      }
    },
...
/**
 * Map DH property type to standard type
 */
...
/**
 * Map DH transaction type to standard type
 */
 ...
 */
function extractFeatures(listing: DHListing): string[] {
  ...
  if (listing.isNew) features.push('new_listing');
  if (listing.isExclusive) features.push('exclusive');
  if (listing.enabledOtthonStart) features.push('otthon_start_eligible');
