...
function createChecksum(listing: IngatlannetListing): { portal: string; portalId: string; contentHash: string } {
  const checksumData = [
    listing.id,
    listing.price,
    listing.title,
  ...
  return {
    ...
    contentHash: crypto.createHash('md5').update(checksumData).digest('hex'),
  };
}
...
export async function runTwoPhaseScrape(
  ...
  // Phase 1: Fast scan - collect all listings
  const phase1Start = Date.now();
  console.log(JSON.stringify({ level: 'info', service: 'ingatlannet-hu', msg: 'Phase 1: Fast scan starting' }));
  ...
  console.log(JSON.stringify({ level: 'info', service: 'ingatlannet-hu', msg: 'Phase 1 complete', totalListings: allListings.length, durationMs: stats.phase1.durationMs }));

  if (allListings.length === 0) return stats;

  // Phase 2: Checksum comparison
  const phase2Start = Date.now();
  console.log(JSON.stringify({ level: 'info', service: 'ingatlannet-hu', msg: 'Phase 2: Checksum comparison starting' }));

  const checksums = allListings.map(createChecksum);
  ...
  const comparison = await checksumClient.compareChecksumsInBatches(
    ...
    5000,
    (current, total) => {
      console.log(JSON.stringify({ level: 'info', service: 'ingatlannet-hu', msg: 'Checksum progress', current, total }));
    }
  );
  ...
  console.log(JSON.stringify({ level: 'info', service: 'ingatlannet-hu', msg: 'Phase 2 complete', new: comparison.new, changed: comparison.changed, unchanged: comparison.unchanged, savingsPercent: stats.phase2.savingsPercent }));

  // Store checksums
  try {
    await checksumClient.updateChecksums(checksums, scrapeRunId);
  } catch (error: any) {
    console.error(JSON.stringify({ level: 'error', service: 'ingatlannet-hu', msg: 'Failed to store checksums', err: error.message }));
  }
  ...
  const listingsToProcess = allListings.filter(
    listing => toFetchSet.has(`ingatlannet-${listing.id}`)
  );

  if (listingsToProcess.length > 0) {
    await addDetailJobs(listingsToProcess);
  }

  stats.phase3.queued = listingsToProcess.length;
  stats.phase3.durationMs = Date.now() - phase3Start;

  console.log(JSON.stringify({ level: 'info', service: 'ingatlannet-hu', msg: 'Phase 3 complete', queued: stats.phase3.queued }));
