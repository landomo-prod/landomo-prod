...
| `imageCount` | Portal metadata | number | Stored in metadata | Total image count |
| `condition` | `condition` | string | Normalized value | Property condition rating |
| `conditionRating` | Portal metadata | string | Stored in metadata | Condition rating from portal |
...
| `source_platform` | Constant | "ingatlannet.hu" |
...
| `condition` | `condition` | Normalized condition value |
...
All portal-specific fields are stored in `portal_metadata['ingatlannet.hu']`:
...
- `condition_rating`: Portal's condition assessment
...
- **Medium**: address, images, publication date, condition
- **Low**: agent info, floor, year built, coordinates
- **Missing**: heating, construction type, furnished, deposit, utilities, renovation year
