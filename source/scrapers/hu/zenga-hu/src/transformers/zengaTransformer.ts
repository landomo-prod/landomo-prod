...
import { ZengaListing, PROPERTY_TYPE_MAP, TRANSACTION_TYPE_MAP } from '../types/zengaTypes';

/**
 * Transform Zenga.hu listing to StandardProperty format
 */
export function transformZengaToStandard(listing: ZengaListing): StandardProperty & Record<string, any> {
  ...
  return {
    ...
    details: {
      ...
      renovation_year: undefined, // Zenga.hu doesn't provide renovation year separately
      parking_spaces: listing.parking ? 1 : undefined,
    },

    // Financial details
    price_per_sqm: listing.pricePerSqm || (listing.price && sqm ? Math.round(listing.price / sqm) : undefined),
    ...
    available_from: undefined, // Zenga.hu doesn't provide available_from
...
/**
 * Map Zenga.hu property type to standard type
 */
...
/**
 * Map Zenga.hu transaction type to standard type
 */
 ...
 */
function extractFeatures(listing: ZengaListing): string[] {
  ...
  if (listing.isPremier) features.push('zenga_premier');
