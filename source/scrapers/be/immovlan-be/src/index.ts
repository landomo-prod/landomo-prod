...
const PORTAL = 'immovlan';

const log = createLogger({ service: 'immovlan-scraper', portal: PORTAL, country: 'belgium' });
...
app.get('/health', async (req, res) => {
  ...
  res.json({ status: 'healthy', scraper: PORTAL, version: '1.0.0', workers: WORKER_CONCURRENCY, queue: queueStats, timestamp: new Date().toISOString() });
});
...
async function runScraper() {
  ...
  try {
    ...
    while (true) {
      ...
      if (remaining !== lastCount) { log.info({ waiting: stats.waiting, active: stats.active, completed: stats.completed }, 'Queue progress'); lastCount = remaining; }
      if (remaining === 0 && stats.active === 0) break;
      await new Promise(resolve => setTimeout(resolve, 2000));
    }

    const finalStats = await getQueueStats();
    await tracker.complete({ listings_found: phaseStats.phase1.totalListings, listings_new: phaseStats.phase2.new, listings_updated: phaseStats.phase2.changed });
    ...
    log.info({ durationMin: (durationSec / 60).toFixed(2), totalFound: phaseStats.phase1.totalListings, processed: finalStats.completed }, 'Scrape completed');
  } catch (error: any) {
    await tracker.fail();
...
app.listen(PORT, '0.0.0.0', () => { log.info({ port: PORT }, 'Immovlan scraper running'); });

process.on('SIGTERM', async () => { await workers.close(); await detailQueue.close(); process.exit(0); });
process.on('SIGINT', async () => { await workers.close(); await detailQueue.close(); process.exit(0); });
