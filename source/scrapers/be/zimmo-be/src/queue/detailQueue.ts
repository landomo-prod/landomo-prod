...
export const detailQueue = new Queue('zimmo-details', {
...
let batch: any[] = [];
const BATCH_SIZE = 50;
const adapter = new IngestAdapter('zimmo');
...
async function flushBatch() {
  ...
  try {
    ...
    batch = [];
    console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Sent batch', count: batchSize }));
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', service: 'zimmo-scraper', msg: 'Failed to send batch', err: (error as any)?.message }));
  }
}
...
export function createDetailWorker(concurrency: number = 30) {
  const worker = new Worker<DetailJob>(
    ...
    async (job: Job<DetailJob>) => {
      ...
      try {
        ...
        console.error(JSON.stringify({ level: 'error', service: 'zimmo-scraper', msg: 'Failed to process', id, err: error.message }));
        throw error;
      }
    },
  ...
  worker.on('failed', (job: any, err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'zimmo-scraper', msg: 'Job failed', id: job?.data.id, err: err.message }));
  });
...
export async function addDetailJobs(jobs: DetailJob[]) {
  ...
  console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Queued detail jobs', count: jobs.length }));
}
