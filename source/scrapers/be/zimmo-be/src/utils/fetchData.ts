  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (axiosError.response?.status === 429) {
        ...
        console.log(JSON.stringify({ level: 'warn', service: 'zimmo-scraper', msg: 'Rate limited (429)', retryAfter }));
      ...
      console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Retrying', attempt: attempt + 1, delayMs }));
      await delay(delayMs);
    }
  }
};
...
export const fetchListingDetail = async (id: string | number): Promise<DetailFetchResult> => {
  const url = `https://www.zimmo.be/en/property/${id}`;
  const headers = getRealisticHeaders('https://www.zimmo.be/');
...
export const fetchAllListingPages = async (
  ...
  const typeSlug = transactionType === 'sale' ? 'te-koop' : 'te-huur';
  const categorySlug = getCategorySlug(category);

  console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Fetching listings', category, transactionType }));
  ...
  console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Fetched total listings', total: allListings.length }));
  return allListings;
};

async function fetchSearchPage(categorySlug: string, typeSlug: string, page: number): Promise<any[]> {
  const url = `https://www.zimmo.be/nl/${typeSlug}/${categorySlug}/?pagina=${page}`;
  const headers = getRealisticHeaders('https://www.zimmo.be/');
  ...
  try {
    ...
    // Extract listings from embedded data
    ...
    // Extract from HTML cards as fallback
    const listings: any[] = [];
    const cardMatches = html.matchAll(/data-property-id="(\d+)"/g);
    ...
    console.error(JSON.stringify({ level: 'error', service: 'zimmo-scraper', msg: 'Search page failed', page, err: (error as any).message }));
    return [];
  }
}
