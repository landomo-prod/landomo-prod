...
import { batchCreateZimmoChecksums } from '../utils/checksumExtractor';
...
export async function runThreePhaseScrape(scrapeRunId?: string): Promise<PhaseStats> {
  ...
  const results = await Promise.allSettled(
    combos.map(({ category, transactionType }) => limit(async () => {
      console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Fetching category', category, type: transactionType }));
      ...
      const checksums = batchCreateZimmoChecksums(listings);
      ...
      try { await checksumClient.updateChecksums(checksums, scrapeRunId); } catch (e: any) {
        console.error(JSON.stringify({ level: 'error', service: 'zimmo-scraper', msg: 'Checksum store failed', err: e.message }));
      }
      ...
      console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Category done', category, type: transactionType, new: comparison.new, changed: comparison.changed, unchanged: comparison.unchanged }));
    }))
  );

  for (const result of results) {
    if (result.status === 'rejected') {
      console.error(JSON.stringify({ level: 'error', service: 'zimmo-scraper', msg: 'Category combo failed', err: result.reason?.message }));
    }
  }
...
export function printThreePhaseSummary(stats: PhaseStats): void {
  console.log(JSON.stringify({ level: 'info', service: 'zimmo-scraper', msg: 'Summary', ...stats }));
}
