# Logic-Immo BE Scraper Configuration
...
be-logic-immo:
  build:
    context: .
    dockerfile: scrapers/Belgium/logic-immo-be/Dockerfile
  ports:
    - "8213:8213"
  environment:
    PORT: 8213
...
Note: Logic-Immo uses `cheerio` for HTML parsing (unlike immoweb/zimmo which use regex).
