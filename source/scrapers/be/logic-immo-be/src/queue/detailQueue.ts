...
export const detailQueue = new Queue('logic-immo-be-details', {
...
let batch: any[] = [];
const BATCH_SIZE = 50;
const adapter = new IngestAdapter('logic-immo-be');
...
async function flushBatch() {
  ...
  try {
    ...
    console.error(JSON.stringify({ level: 'error', service: 'logic-immo-be-scraper', msg: 'Failed to send batch', err: error.message }));
  }
}
...
export function createDetailWorker(concurrency: number = 50) {
  const worker = new Worker<DetailJob>(
    ...
    async (job: Job<DetailJob>) => {
      ...
      batch.push({
        portalId: `logic-immo-be-${listingId}`,
        data: transformed,
        rawData: detail,
      });
  ...
  worker.on('failed', (job: any, err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'logic-immo-be-scraper', msg: 'Job failed', listingId: job?.data?.listingId, err: err.message }));
  });

  worker.on('error', (err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'logic-immo-be-scraper', msg: 'Worker error', err: err?.message }));
  });
...
export async function addDetailJobs(jobs: DetailJob[]) {
  ...
  console.log(JSON.stringify({ level: 'info', service: 'logic-immo-be-scraper', msg: 'Queued detail jobs', count: jobs.length }));
}
