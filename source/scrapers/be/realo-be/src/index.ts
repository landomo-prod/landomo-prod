...
const app = express();
const PORT = parseInt(process.env.PORT || '8214', 10);
const PORTAL = 'realo-be';

const log = createLogger({ service: 'realo-be-scraper', portal: PORTAL, country: 'belgium' });
...
app.listen(PORT, '0.0.0.0', () => {
  log.info({ port: PORT, workers: WORKER_CONCURRENCY }, 'Realo BE scraper running');
});
