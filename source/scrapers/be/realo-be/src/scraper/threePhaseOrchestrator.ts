...
export async function runThreePhaseScrape(scrapeRunId?: string): Promise<PhaseStats> {
  ...
  console.log(JSON.stringify({ level: 'info', service: 'realo-be-scraper', msg: 'Phase 1: Fetching all listings' }));
  ...
  console.log(JSON.stringify({ level: 'info', service: 'realo-be-scraper', msg: 'Phase 1 complete', totalListings: listings.length, durationMs: stats.phase1.durationMs }));

  if (listings.length === 0) return stats;

  const phase2Start = Date.now();
  console.log(JSON.stringify({ level: 'info', service: 'realo-be-scraper', msg: 'Phase 2: Comparing checksums' }));
  ...
  const comparison = await checksumClient.compareChecksumsInBatches(
    ...
    5000,
    (current, total) => {
      console.log(JSON.stringify({ level: 'info', service: 'realo-be-scraper', msg: 'Checksum progress', current, total }));
    }
  );
  ...
  try {
    ...
    console.error(JSON.stringify({ level: 'error', service: 'realo-be-scraper', msg: 'Failed to store checksums', err: error.message }));
  }

  console.log(JSON.stringify({ level: 'info', service: 'realo-be-scraper', msg: 'Phase 2 complete', new: comparison.new, changed: comparison.changed, unchanged: comparison.unchanged }));
  ...
  console.log(JSON.stringify({ level: 'info', service: 'realo-be-scraper', msg: 'Phase 3 complete', queued: jobs.length }));
