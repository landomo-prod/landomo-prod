...
const BASE_URL = 'https://www.realo.be';

const SEARCH_PATHS: { category: string; transaction: string; path: string }[] = [
  { category: 'apartment', transaction: 'sale', path: '/nl/te-koop/appartement' },
  { category: 'apartment', transaction: 'rent', path: '/nl/te-huur/appartement' },
  { category: 'house', transaction: 'sale', path: '/nl/te-koop/huis' },
  { category: 'house', transaction: 'rent', path: '/nl/te-huur/huis' },
  { category: 'land', transaction: 'sale', path: '/nl/te-koop/grond' },
  { category: 'commercial', transaction: 'sale', path: '/nl/te-koop/commercieel' },
  { category: 'commercial', transaction: 'rent', path: '/nl/te-huur/commercieel' },
];

export async function fetchAllListingPages(
  categoryFilter?: string[]
): Promise<RawRealoListing[]> {
  const allListings: RawRealoListing[] = [];
  ...
  const results = await Promise.allSettled(
    paths.map(searchPath => limit(async () => {
      const pathListings: RawRealoListing[] = [];
      let page = 1;
      let hasMore = true;
      ...
      while (hasMore) {
        try {
          ...
          if (axiosError.response?.status === 429) {
            ...
            console.log(JSON.stringify({ level: 'warn', service: 'realo-be-scraper', msg: 'Rate limited (429)', retryAfter }));
  ...
  for (const result of results) {
    if (result.status === 'fulfilled') {
      ...
      console.error(JSON.stringify({ level: 'error', service: 'realo-be-scraper', msg: 'Path failed', err: result.reason?.message }));
    }
  }
...
function parseListingsFromHTML(
  ...
  const listings: RawRealoListing[] = [];
  ...
  // Try embedded JSON (Realo may use GraphQL with embedded state)
  $('script').each((_, el) => {
    const text = $(el).html() || '';
    if (text.includes('__APOLLO_STATE__') || text.includes('window.__DATA__')) {
      try {
        const match = text.match(/(?:__APOLLO_STATE__|window\.__DATA__)\s*=\s*({.+?});?\s*(?:<\/script>|$)/s);
        if (match) {
          const data = JSON.parse(match[1]);
          const properties = extractPropertiesFromApolloState(data, category, transaction);
  ...
  $('[data-property-id], .property-card, .listing-card, .search-result').each((_, el) => {
    const $el = $(el);
    const id = $el.attr('data-property-id') || $el.attr('data-id') || $el.find('a').first().attr('href')?.match(/\/(\d+)/)?.[1];
    if (!id) return;
    ...
    const bedroomsText = $el.find('[class*="bedroom"], [class*="slaapkamer"]').first().text().replace(/[^\d]/g, '');
    ...
    listings.push({
      ...
      address: {
        city: $el.find('[class*="location"], [class*="city"], [class*="gemeente"]').first().text().trim() || undefined,
      },
...
function extractPropertiesFromApolloState(state: any, category: string, transaction: string): RawRealoListing[] {
  const listings: RawRealoListing[] = [];
    ...
    if (key.startsWith('Property:') || key.startsWith('Listing:')) {
      ...
      if (item && item.id) {
        listings.push({
          ...
          type: category,
          transaction_type: transaction,
          surface: item.surface || item.livingArea,
          bedrooms: item.bedrooms || item.numberOfBedrooms,
          bathrooms: item.bathrooms,
          address: {
            city: item.address?.city || item.city,
            postal_code: item.address?.postalCode || item.postalCode,
...
function mapNextDataListing(item: any, category: string, transaction: string): RawRealoListing | null {
  ...
  return {
    ...
    plot_surface: item.plotSurface || item.plotArea,
...
function mapLdJsonListing(item: any, category: string, transaction: string): RawRealoListing | null {
...
export async function fetchListingDetail(
  listingId: string,
  url?: string
): Promise<RawRealoListing | null> {
  try {
    const detailUrl = url?.startsWith('http') ? url : `${BASE_URL}${url || `/nl/detail/${listingId}`}`;
    const headers = getRealisticHeaders(BASE_URL);
...
function parseDetailFromHTML(html: string, listingId: string): RawRealoListing | null {
  const $ = cheerio.load(html);
  ...
  // Try Apollo state
  let detail: RawRealoListing | null = null;
  $('script').each((_, el) => {
    const text = $(el).html() || '';
    if (text.includes('__APOLLO_STATE__') || text.includes('window.__DATA__')) {
      try {
        ...
        if (match) {
          const data = JSON.parse(match[1]);
          const properties = extractPropertiesFromApolloState(data, 'apartment', 'sale');
          if (properties.length > 0) detail = properties[0];
        }
      } catch (e) { /* skip */ }
    }
  });
