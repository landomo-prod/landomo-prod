...
export const detailQueue = new Queue('immoweb-details', {
...
let batch: any[] = [];
const BATCH_SIZE = 50;
const adapter = new IngestAdapter('immoweb');
...
async function flushBatch() {
  ...
  try {
    ...
    batch = [];
    console.log(JSON.stringify({ level: 'info', service: 'immoweb-scraper', msg: 'Sent batch', count: batchSize }));
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', service: 'immoweb-scraper', msg: 'Failed to send batch', err: (error as any)?.message }));
  }
}
...
function transformListing(raw: any, category: string, transactionType: string): any {
  ...
  if (cat === 'OFFICE' || cat === 'COMMERCIAL' || cat === 'INDUSTRY') return transformCommercial(raw, transactionType);
...
export function createDetailWorker(concurrency: number = 50) {
  const worker = new Worker<DetailJob>(
    ...
    async (job: Job<DetailJob>) => {
      ...
      try {
        ...
        // If we don't have full listing data, fetch detail
        if (!data || !data.property?.description) {
          const result = await fetchListingDetail(id);
          if (result.isInactive) {
            return { skipped: true, reason: result.inactiveReason };
          }
          data = result.data;
        }
        ...
        console.error(JSON.stringify({ level: 'error', service: 'immoweb-scraper', msg: 'Failed to process', id, err: error.message }));
        throw error;
      }
    },
    {
  ...
  worker.on('failed', (job: any, err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'immoweb-scraper', msg: 'Job failed', id: job?.data.id, err: err.message }));
  });

  worker.on('error', (err: any) => {
    console.error(JSON.stringify({ level: 'error', service: 'immoweb-scraper', msg: 'Worker error', err: err?.message }));
  });
...
export async function addDetailJobs(jobs: DetailJob[]) {
  ...
  console.log(JSON.stringify({ level: 'info', service: 'immoweb-scraper', msg: 'Queued detail jobs', count: jobs.length }));
}
