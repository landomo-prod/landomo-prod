...
const PORTAL = 'nieruchomosci-online';

const log = createLogger({ service: 'nieruchomosci-online-scraper', portal: PORTAL, country: 'poland' });
 ...
 * Query params:
 *   ?categories=mieszkanie,dom - Scrape specific categories
 *   ?categories=critical - Scrape critical (mieszkanie, dom)
 *   ?categories=standard - Scrape standard (dzialka, lokal)
 */
app.post('/scrape', async (req, res) => {
  ...
  if (categoriesParam) {
    if (categoriesParam === 'critical') {
      categories = ['mieszkanie', 'dom'];
    } else if (categoriesParam === 'standard') {
      categories = ['dzialka', 'lokal'];
    } else {
...
app.listen(PORT, '0.0.0.0', () => {
  log.info({ port: PORT, workers: WORKER_CONCURRENCY }, 'Nieruchomosci-Online scraper running');
});
