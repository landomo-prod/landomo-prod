...
export class IngestAdapter {
  ...
  constructor(portal: string) {
    ...
    this.apiKey = process.env.INGEST_API_KEY || 'dev_key_pl_1';
  }
  ...
  async sendProperties(properties: PropertyPayload[]): Promise<void> {
    ...
    console.log(JSON.stringify({ level: 'info', service: 'nieruchomosci-online-scraper', msg: 'Batch sent to ingest', count: properties.length }));
