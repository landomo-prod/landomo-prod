...
import { batchCreateImmoScout24ATChecksums } from '../utils/checksumExtractor';
...
export async function runThreePhaseScrape(scrapeRunId?: string): Promise<PhaseStats> {
  ...
  for (const category of SEARCH_CATEGORIES) {
    console.log(JSON.stringify({ level: 'info', service: 'immoscout24-at', msg: 'Fetching category', category }));
    ...
    console.log(JSON.stringify({ level: 'info', service: 'immoscout24-at', msg: 'Category fetched', category, count: listings.length }));

    if (listings.length === 0) continue;

    // Phase 2: compare checksums
    const phase2Start = Date.now();
    const checksums = batchCreateImmoScout24ATChecksums(listings);
    ...
    const comparison = await checksumClient.compareChecksumsInBatches(
      ...
      5000,
      (current, total) => {
        console.log(JSON.stringify({ level: 'info', service: 'immoscout24-at', msg: 'Checksum progress', category, current, total }));
      }
    );
    ...
    try {
      ...
      console.error(JSON.stringify({ level: 'error', service: 'immoscout24-at', msg: 'Failed to store checksums', category, err: error.message }));
    }
    ...
    console.log(JSON.stringify({ level: 'info', service: 'immoscout24-at', msg: 'Category processed', category, new: comparison.new, changed: comparison.changed, unchanged: comparison.unchanged, queued: jobs.length }));
  }
  ...
  console.log(JSON.stringify({ level: 'info', service: 'immoscout24-at', msg: 'Three-phase complete', totalListings: stats.phase1.totalListings, new: stats.phase2.new, changed: stats.phase2.changed, unchanged: stats.phase2.unchanged, savingsPercent: stats.phase2.savingsPercent, queued: stats.phase3.queued }));
