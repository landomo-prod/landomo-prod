...
/**
 * Transform Reality.sk listing to StandardProperty format
 *
 * Reality.sk is an HTML-only scraper (list pages). We extract as much as possible
 * from the title, description, and params text via pattern matching.
 */
export function transformRealityToStandard(listing: RealityListing): StandardProperty & Record<string, any> {
  ...
  const parsedHeating = extractHeatingFromText(allText);
  const parsedFurnished = extractFurnishedFromText(allText);
  const parsedConstructionType = extractConstructionTypeFromText(allText);
  const parsedFloor = extractFloorFromText(allText);
  const parsedTotalFloors = extractTotalFloorsFromText(allText);
  const parsedYearBuilt = extractYearBuiltFromText(allText);
  const parsedRenovationYear = extractRenovationYearFromText(allText);
  const parsedEnergyRating = normalizeEnergyRating(extractEnergyRatingFromText(allText));
  const parsedDeposit = extractDepositFromText(allText);
  const parsedAreaLiving = listing.sqm;
  ...
  return {
    ...
    // Financial details
    price_per_sqm: listing.price && listing.sqm ? Math.round(listing.price / listing.sqm) : undefined,

    // Amenities (structured booleans)
    amenities: {
      has_parking: amenities.has_parking || undefined,
      has_garage: amenities.has_garage || undefined,
      has_balcony: amenities.has_balcony || undefined,
      has_terrace: amenities.has_terrace || undefined,
      has_elevator: amenities.has_elevator || undefined,
      has_basement: amenities.has_basement || undefined,
      has_garden: amenities.has_garden || undefined,
      has_pool: amenities.has_pool || undefined,
      has_fireplace: amenities.has_fireplace || undefined,
      has_ac: amenities.has_ac || undefined,
      has_loggia: amenities.has_loggia || undefined,
      is_furnished: furnishedEn === 'furnished' || undefined,
      is_new_construction: conditionEn === 'new' || undefined,
    },
    ...
    condition: conditionEn as SlovakSpecificFields['condition'],
    heating_type: heatingEn as SlovakSpecificFields['heating_type'],
    furnished: furnishedEn,
    construction_type: constructionEn as SlovakSpecificFields['construction_type'],
    available_from: undefined, // Not available from list page HTML
    published_date: undefined, // Not available from list page HTML
    ...
    country_specific: {
      disposition: listing.rooms ? normalizeDisposition(`${listing.rooms}-izbový`) : undefined,
      ownership: 'other' as const,
      condition: conditionEn as SlovakSpecificFields['condition'],
      furnished: furnishedEn as SlovakSpecificFields['furnished'],
      energy_rating: parsedEnergyRating,
      heating_type: heatingEn as SlovakSpecificFields['heating_type'],
      construction_type: constructionEn as SlovakSpecificFields['construction_type'],
    ...
    ...({ slovak_disposition: listing.rooms ? normalizeDisposition(`${listing.rooms}-izbový`) : undefined, slovak_ownership: normalizeOwnership(undefined) } as Record<string, unknown>),
...
/**
 * Map Reality.sk property type to standard type
 *
 * SAFETY FIX: Commercial/recreational/garage mapped to 'house' to prevent transformation errors
...
function mapPropertyType(propertyType: string): string {
  const typeMap: Record<string, string> = {
    ...
    'domy': 'house',
    'pozemky': 'land',
    // Safety mappings to prevent data loss (buildings with land → house)
    'chaty-chalupy': 'house',  // recreational properties (cabins, cottages)
    'kancelarie': 'house',     // commercial offices
    'garaze': 'house'          // garages
  };
  return typeMap[propertyType.toLowerCase()] || 'house';  // Default to house
}

/**
 * Map Reality.sk transaction type to standard type
 */
function mapTransactionType(transactionType: string): 'sale' | 'rent' {
  const typeMap: Record<string, 'sale' | 'rent'> = {
    'predaj': 'sale',
    'prenajom': 'rent',
    'drazba': 'sale' // Auction counts as sale
  };
...
// Parse structured data from title + description text
// ============================================================================
 ...
 */
function extractHeatingFromText(text: string): string | undefined {
  if (!text) return undefined;
  const lower = text.toLowerCase();
 ...
 */
function extractFurnishedFromText(text: string): string | undefined {
  if (!text) return undefined;
  const lower = text.toLowerCase();
 ...
 */
function extractConstructionTypeFromText(text: string): string | undefined {
  if (!text) return undefined;
  const lower = text.toLowerCase();
 ...
 */
function extractFloorFromText(text: string): number | undefined {
  if (!text) return undefined;
  const lower = text.toLowerCase();
 ...
 */
function extractTotalFloorsFromText(text: string): number | undefined {
  if (!text) return undefined;
 ...
 */
function extractYearBuiltFromText(text: string): number | undefined {
  if (!text) return undefined;
 ...
 */
function extractRenovationYearFromText(text: string): number | undefined {
  if (!text) return undefined;
 ...
 */
function extractEnergyRatingFromText(text: string): string | undefined {
  if (!text) return undefined;
 ...
 */
function extractDepositFromText(text: string): number | undefined {
  if (!text) return undefined;
 ...
 */
function extractAreaPlotFromText(text: string): number | undefined {
  if (!text) return undefined;
 ...
 */
function extractAmenitiesFromText(text: string): Record<string, boolean> {
  if (!text) return {};
  const lower = text.toLowerCase();
