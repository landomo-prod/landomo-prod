...
/**
 * Transform TopReality.sk listing to StandardProperty format
 *
 * TopReality.sk is an HTML scraper (list pages). We extract as much as possible
 * from the title, description, and raw listing text via pattern matching.
 */
export function transformTopRealityToStandard(listing: TopRealityListing): StandardProperty & Record<string, any> {
...
/**
 * Map TopReality.sk property type to standard type
 *
...
function mapPropertyType(propertyType: string): string {
  const typeMap: Record<string, string> = {
    ...
    'rekreačné': 'house',     // recreational properties
    'garáže': 'house',        // garages
    'iné': 'house'            // other/unknown → default to house
  };
  return typeMap[propertyType.toLowerCase()] || 'house';
}
