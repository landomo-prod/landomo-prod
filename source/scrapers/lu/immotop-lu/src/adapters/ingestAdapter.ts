...
export class IngestAdapter {
  ...
  async sendProperties(properties: PropertyPayload[]): Promise<void> {
    ...
    try {
      ...
      console.log(JSON.stringify({ level: 'info', service: 'immotop-scraper', msg: 'Batch sent to ingest', count: properties.length }));
    } catch (error: any) {
      console.error(JSON.stringify({ level: 'error', service: 'immotop-scraper', msg: 'Failed to send batch', count: properties.length, err: error.message, status: error.response?.status }));
      throw error;
    }
  }
}
