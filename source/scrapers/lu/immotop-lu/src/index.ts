...
const PORTAL = 'immotop';

const log = createLogger({ service: 'immotop-scraper', portal: PORTAL, country: 'luxembourg' });

// Lower concurrency for HTML scraping (polite crawling)
const WORKER_CONCURRENCY = parseInt(process.env.WORKER_CONCURRENCY || '20');
...
app.listen(PORT, '0.0.0.0', () => {
  log.info({ port: PORT, workers: WORKER_CONCURRENCY }, 'Immotop.lu scraper running');
});
