...
export class IngestAdapter {
  ...
  constructor(portal: string) {
    ...
    this.apiKey = process.env[`INGEST_API_KEY_${portal.toUpperCase().replace(/-/g, '_')}`] || process.env.INGEST_API_KEY || 'dev_key_nl_1';
  }
  ...
  async sendProperties(properties: PropertyPayload[]): Promise<void> {
    ...
    try {
      ...
      console.log(JSON.stringify({ level: 'info', service: 'pararius-scraper', msg: 'Batch sent to ingest', count: properties.length }));
      return response.data;
    } catch (error: any) {
      console.error(JSON.stringify({ level: 'error', service: 'pararius-scraper', msg: 'Failed to send batch', count: properties.length, err: error.message, status: error.response?.status }));
      throw error;
    }
  }
