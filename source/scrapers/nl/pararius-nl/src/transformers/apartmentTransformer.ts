...
export function transformToApartment(raw: ParariusDetailData): ApartmentPropertyTierI {
  const furnished = raw.furnished?.toLowerCase().includes('gemeubileerd') ? 'furnished' as const
    : raw.furnished?.toLowerCase().includes('gestoffeerd') ? 'partially_furnished' as const
    : raw.furnished ? 'not_furnished' as const : undefined;
  ...
  return {
    ...
    furnished,
    available_from: raw.availableFrom,
    deposit: raw.deposit,
