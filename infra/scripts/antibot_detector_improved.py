#!/usr/bin/env python3
"""
Improved anti-bot blocker detection script for real estate portals.
Extracts ALL domains from markdown files and creates folder structure.
"""
...
class AntiBotDetector:
    ...
    def _detect_generic_waf(self, headers: Dict, content: str) -> bool:
        ...
        waf_patterns = [
            ...
            'coming soon' in content and len(content) < 1000,
        ]
        return any(waf_patterns)


def extract_domains_from_markdown_improved(scrapers_dir: str) -> Dict[str, List[str]]:
    """
    Extract ALL domains from markdown files with improved parsing.
    Handles various markdown table formats and extracts all URLs.
    """
    domains_by_country = {}
    ...
    for country_dir in os.listdir(scrapers_dir):
        ...
        try:
            ...
            # Extract all URLs/domains from markdown (various formats)
            # Pattern 1: Standard markdown link format [text](url)
            urls_pattern1 = re.findall(r'\[.*?\]\((https?://[^\)]+|www\.[^\)]+)\)', content)
            domains.extend(urls_pattern1)

            # Pattern 2: Direct URLs in text
            urls_pattern2 = re.findall(r'https?://[^\s\)\|\n]+|www\.[^\s\)\|\n]+', content)
            domains.extend(urls_pattern2)

            # Pattern 3: Table cells (between pipes)
            lines = content.split('\n')
            for line in lines:
                if line.strip().startswith('|') and not line.strip().startswith('|--'):
                    ...
                    for part in parts[1:-1]:  # Skip first and last empty parts
                        # Look for URLs in this cell
                        urls = re.findall(r'https?://[^\s\)]+|www\.[^\s\)]+', part)
                        domains.extend(urls)
            ...
            if domains:
                ...
                unique_domains = [d for d in unique_domains if len(d) > 5]
                domains_by_country[country_dir] = sorted(unique_domains)
...
def slugify_domain(url: str) -> str:
    ...
    try:
        ...
        if ']' in url and not url.startswith('http'):
            # Fix malformed markdown links like "www.example.com](https://..."
        ...
        # Fallback: clean URL string directly
        url = re.sub(r'[^a-zA-Z0-9.\-]', '', url)
        return url.replace('www.', '').replace('.', '-').lower()
...
def main():
    ...
    # Extract all domains with improved parsing
    print("Extracting domains from markdown files (improved)...")
    domains_by_country = extract_domains_from_markdown_improved(scrapers_dir)
    ...
    for country in sorted(domains_by_country.keys()):
        domain_list = domains_by_country[country]
    ...
    report_file = os.path.join(scrapers_dir, 'antibot_detection_report_improved.json')
